...
static int
cmphrefs (const void *v1, const void *v2)
{
  return (strcmp (((const DPS_HREF *) v1)->url, ((const DPS_HREF *) v2)->url));
}

__C_LINK int __DPSCALL
DpsHrefListAdd (DPS_AGENT *A, DPS_HREFLIST *HrefList, DPS_HREF *Href)
{
  int l, r, c, res;
  size_t i, len;
  char *ehref, *s, *e, *ss;
  ...
  if ((s = strstr (ehref, "://")) != NULL)
    {
      if ((e = strchr (s + 3, '/')) != NULL)
        {
          if ((ss = strchr (s + 3, '@')) == NULL)
            ss = s + 3;
          for (; ss < e; ss++)
            if (*ss >= 'A' && *ss <= 'Z')
              *ss = (char) dps_tolower ((int) *ss);
        }
    }

  /* Find current URL in sorted part of list */
  l = 0;
  r = (int) HrefList->shrefs - 1;
  while (l <= r)
    {
      c = (l + r) / 2;
...
extern __C_LINK void __DPSCALL
DpsHrefListFree (DPS_HREFLIST *HrefList)
{
  size_t i;
...
__C_LINK DPS_HREFLIST *__DPSCALL
DpsHrefListInit (DPS_HREFLIST *Hrefs)
{
