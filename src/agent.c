...
__C_LINK int
DpsAgentStoredConnect (DPS_AGENT *Indexer)
{
  size_t i;
...
DPS_DBLIST *
DpsAgentDBLSet (DPS_AGENT *result, DPS_ENV *Env)
{
  DPS_DBLIST *DBL = NULL;
  size_t i, j;
...
__C_LINK DPS_AGENT *__DPSCALL
DpsAgentInit (DPS_AGENT *result, DPS_ENV *Env, int handle)
{
  size_t i, z;
  DPS_DBLIST *DBL;
  char port_str[16];
  ...
  if (!loccs)
    loccs = DpsGetCharSet (DpsVarListFindStr (&Env->Vars, "LocalCharset", "iso-8859-1"));
...
DpsAgentFree (DPS_AGENT *Indexer)
{
  size_t i;
  ...
  for (i = 0; i < DPS_FINDURL_CACHE_SIZE; i++)
    DPS_FREE (Indexer->DpsFindURLCache[i]);
  for (i = 0; i < DPS_SERVERID_CACHE_SIZE; i++)
    DPS_FREE (Indexer->ServerIdCache[i].Match_Pattern);
  if (Indexer->freeme)
    DPS_FREE (Indexer);
}

__C_LINK void __DPSCALL
DpsAgentSetAction (DPS_AGENT *Indexer, int action)
{
