...
static int
do_RESTful (DPS_AGENT *Agent, int client, const DPS_SEARCHD_PACKET_HEADER *hdr)
{
  ...
  while (*p != '\0' && *p != ' ')
    p++; /* skip command name, it's only GET implemented */
  if (*p == ' ')
    p++;
  if ((pp = strchr (p, (int) '?')) != NULL)
    p = pp + 1;
  if (*p == '\0')
    return DPS_ERROR;
  ...
  if ((strncmp (template_name, conf_dir, dps_strlen (conf_dir)) || (res = DpsTemplateLoad (Agent, Env, &Agent->tmpl, template_name))))
    {
      ...
      if (strcmp (template_name, "search.htm"))
        { /* trying load default template */
          DPS_FREE (template_filename);
...
static int
do_client (DPS_AGENT *Agent, int client)
{
  char buf[1024] = "";
      ...
      switch (last_cmd = hdr.cmd)
...
#endif
          if (!dinfo)
            dinfo = (char *) DpsStrdup ("nodocinfo");
        ...
        case DPS_SEARCHD_CMD_CLONES:
          {
            ...
            if (!dinfo)
              dinfo = (char *) DpsStrdup ("nocloneinfo");
        ...
        case DPS_SEARCHD_CMD_WORDS_ALL:
...

          {
            ...
            for (i = 0; i < Res->WWList.nwords; i++)
              {
                hdr.len += sizeof (DPS_WIDEWORD_EX) + sizeof (char) * (Res->WWList.Word[i].len + 1) + sizeof (dpsunicode_t) * (Res->WWList.Word[i].ulen + 1) + sizeof (int);
              }
            if (hdr.len & 1)
              hdr.len++;
...
static int
client_main (DPS_ENV *Env, size_t handle)
{
  struct timeval tval;
  int ns;
  ...
  if (MaxClients > DPS_CHILDREN_LIMIT)
    MaxClients = DPS_CHILDREN_LIMIT;
  ...
  if (clt_sock > 0)
    dps_closesocket (clt_sock);
  if (sockfd > 0)
    dps_closesocket (sockfd);
              ...
              if (dir)
                {
                  while ((item = readdir (dir)))
                    {
                      if (strncmp (item->d_name, "track.", 6) && strncmp (item->d_name, "query.", 6))
                        continue;
                      to_delete = 0;
                          ...
                          if (n > 0)
                            {
                              ...
                              if (strncmp (item->d_name, "track.", 6))
                              ...
                              else
                                {
                                  ...
                                  if (res != DPS_OK)
                                    {
                                      to_delete = tr_db->connected;
                                      goto tr_loop_continue;
                                    }
      ...
      if (errno == EEXIST)
        {
          ...
          if (0 != ftruncate (pid_fd, 0L))
            {
              fprintf (stderr, "Can't truncate '%s' error %d (%s): %s:%d\n", dps_pid_name, errno, strerror (errno), __FILE__, __LINE__);
              DpsClose (pid_fd);
              exit (1);
            }
        }
  ...
  while (!done)
    {
      ...
      if (pvar_dir == NULL)
        var_dir = DpsVarListFindStr (&Conf->Vars, "VarDir", DPS_VAR_DIR);
      else
        var_dir = pvar_dir;
      ...
      if (unix_socket != NULL)
        DpsLog (Agent, verb, "Unix socket: '%s'", unix_socket);
      ...
      if (thr_setconcurrency (16) != NULL)
        { /* FIXME: Does 16 is enough ? */
      ...
      while (!internaldone)
        {
          ...
          if (have_sigpipe)
            { /* FIXME: why this may happens and what to do with it */
