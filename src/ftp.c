...
int
Dps_ftp_get_reply (DPS_CONN *connp)
{
  if (!connp->buf)
    return -1;
...
int
Dps_ftp_read_line (DPS_CONN *connp)
{
...
int
Dps_ftp_connect (DPS_AGENT *Agent, DPS_CONN *connp, const char *hostname, int port, const char *user, const char *passwd, int timeout)
{
  size_t len;
...
int
Dps_ftp_open_control_port (DPS_AGENT *Agent, DPS_CONN *connp)
{
  int code;
...
int
Dps_ftp_open_data_port (DPS_CONN *c, DPS_CONN *d)
{
  char buf[64];
  unsigned char *a, *p;
...
int
Dps_ftp_send_cmd (DPS_CONN *connp, const char *cmd)
{
  char *buf;
  size_t len;
...
int
Dps_ftp_send_data_cmd (DPS_CONN *c, DPS_CONN *d, char *cmd, size_t max_doc_size)
{
  int code, bytes = 0;
...
int
Dps_ftp_login (DPS_CONN *connp, const char *user, const char *passwd)
{
  char *buf;
...
static int
ftp_parse_list (DPS_CONN *connp, const char *path)
{
...
int
Dps_ftp_list (DPS_CONN *c, DPS_CONN *d, const char *path, const char *filename, size_t max_doc_size)
{
  char *cmd;
  size_t len;
...
int
Dps_ftp_get (DPS_CONN *c, DPS_CONN *d, char *path, size_t max_doc_size)
{
  char *cmd;
  size_t len;
...
int
Dps_ftp_mdtm (DPS_CONN *c, char *path)
{
  char *cmd;
  int code;
  size_t len;
...
ssize_t
Dps_ftp_size (DPS_CONN *c, char *path)
{
  char *cmd;
  int code;
  size_t len;
...
int
Dps_ftp_rest (DPS_CONN *c, size_t rest)
{
  char cmd[64];
  int code;
...
int
Dps_ftp_cwd (DPS_CONN *c, const char *path)
{
  char *cmd;
  int code;
  size_t len;
