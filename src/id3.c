...
static int
add_var (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, const char *name, char *val)
{
  DPS_VAR *Sec;
...
static int
id3_add_var (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, const char *name, char *val, int charset, size_t len)
{
  DPS_VAR *Sec;
#if 1
  ...
  switch (charset)
    {
    case 0:
      from_cs = DpsGetCharSet ("iso-8859-1");
      break;
    case 1:
      if ((val[0] == (char) 0xFE) && (val[1] == (char) 0xFF))
        from_cs = DpsGetCharSet ("utf-16be");
      else if (val[0] == (char) 0xFF && val[1] == (char) 0xFE)
        from_cs = DpsGetCharSet ("utf-16le");
      else
        return DPS_OK; /* wrong sequence for UTF16 with BOM */
      val += 2;
      len -= 2;
      break;
    case 2:
      from_cs = DpsGetCharSet ("utf-16be");
      break;
    case 3: /* utf-8 - no recoding required */
      break;
    }
#endif
  if ((Sec = DpsVarListFind (&Doc->Sections, name)))
...
static int
get_tag (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
  char year[5] = "";
  char album[31];
  char artist[31];
...
static int
get_id3v2 (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
  char *ch;
  ...
  if (hdr_len > Doc->Buf.size)
    return DPS_OK;

  ch = buf_in;

  if (ch[5] & 0x40)
    return DPS_OK; /* compression is not defined for this version */
...
static int
get_id3v23 (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
  char *ch;
  char *buf_in = Doc->Buf.content;
  size_t hdr_len = Doc->Buf.content - Doc->Buf.buf;
  ...
  if (ch[5] & 0x20)
    return DPS_OK; /* experimental is not supported */
  ...
  while (((size_t) (ch - buf_in) + 10 < tag_size) && ((size_t) (ch - buf_in) + 10 < cont_len))
    {
      ...
      if (flags & 0x20)
        { /* skip groupping byte if present */
          ch++;
          frame_size--;
        }
...
static int
get_id3v24 (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
  char *ch;
  char *buf_in = Doc->Buf.content;
  size_t hdr_len = Doc->Buf.content - Doc->Buf.buf;
...
int
DpsMP3Parse (DPS_AGENT *A, DPS_DOCUMENT *Doc)
{
