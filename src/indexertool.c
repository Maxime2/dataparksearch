...
int
DpsShowStatistics (DPS_AGENT *Indexer)
{
  int res;
  DPS_STATLIST Stats;
  size_t snum;
...
void
DpsAppendTarget (DPS_AGENT *Indexer, const char *url, const char *lang, const int hops, int parent)
{
      ...
      for (i = Indexer->Conf->Targets.num_rows - 1; i > 0; i--)
        {
          ...
          if ((strcasecmp (DpsVarListFindStr (&Doc->Sections, "URL", ""), url) == 0) && (strcmp (DpsVarListFindStr (&Doc->RequestHeaders, "Accept-Language", ""), lang) == 0))
            {
  ...
  if (*lang != '\0')
    DpsVarListAddStr (&Doc->RequestHeaders, "Accept-Language", lang);
