...
int
DpsAddStackItem (DPS_AGENT *query, DPS_RESULT *Res, DPS_PREPARE_STATE *state, char *word, dpsunicode_t *uword)
{
  ...
  */
  ...
  if (Res->nitems > 0)
    {
      if (state->cmd == DPS_STACK_OR || state->cmd == DPS_STACK_AND || state->cmd == DPS_STACK_NEAR || state->cmd == DPS_STACK_ANYWORD)
        {
          if (Res->items[Res->nitems - 1].cmd == DPS_STACK_AND || Res->items[Res->nitems - 1].cmd == DPS_STACK_OR || Res->items[Res->nitems - 1].cmd == DPS_STACK_NEAR || Res->items[Res->nitems - 1].cmd == DPS_STACK_ANYWORD)
            {
              return DPS_OK;
            }
        }

      if ((Res->nitems > 0) && (state->cmd == DPS_STACK_WORD) && ((Res->items[Res->nitems - 1].cmd == DPS_STACK_WORD) || (Res->items[Res->nitems - 1].cmd == DPS_STACK_RIGHT) || (Res->items[Res->nitems - 1].cmd == DPS_STACK_PHRASE_RIGHT)))
        {
      ...
      if ((Res->nitems > 0) && (state->cmd == DPS_STACK_LEFT) && ((Res->items[Res->nitems - 1].cmd == DPS_STACK_RIGHT) || (Res->items[Res->nitems - 1].cmd == DPS_STACK_PHRASE_RIGHT)))
        {
  ...
  if (state->cmd != DPS_STACK_WORD)
  ...
  else
    {
      ...
      if (state->order > Res->max_order)
        Res->max_order = state->order;
      if (state->order_inquery > Res->max_order_inquery)
        Res->max_order_inquery = state->order;
    }
...
void
DpsStackItemFree (DPS_STACK_ITEM *item)
{
  if (item == NULL)
    return;
  DPS_FREE (item->pbegin);
  item->pbegin = item->plast = item->pcur = NULL;
  DPS_FREE (item->db_pbegin);
  item->db_pbegin = NULL;
  /*  item->count = 0;*/
...
DPS_BOOLSTACK *
DpsBoolStackInit (DPS_BOOLSTACK *s)
{
  if (s == NULL)
    {
...
void
DpsBoolStackFree (DPS_BOOLSTACK *s)
{
  DPS_FREE (s->cstack);
  DPS_FREE (s->astack);
  if (s->freeme)
...
static DPS_STACK_ITEM *
POPARG (DPS_BOOLSTACK *s)
{
  if (s->nastack > 0)
    {
      s->nastack--;
...
static int
PUSHARG (DPS_BOOLSTACK *s, DPS_STACK_ITEM *arg)
{
...
static int
PUSHCMD (DPS_BOOLSTACK *s, int arg)
{
...
static int
proceedOR (DPS_AGENT *query, DPS_STACK_ITEM *res, DPS_STACK_ITEM *x1, DPS_STACK_ITEM *x2)
{
  ...
  x1->pcur = x1->pbegin;
  x1->plast = x1->pbegin + x1->count;
  x2->pcur = x2->pbegin;
  x2->plast = x2->pbegin + x2->count;
...
static int
proceedSTOP (DPS_AGENT *query, DPS_STACK_ITEM *res, DPS_STACK_ITEM *x, DPS_STACK_ITEM *stop)
{
  ...
  x->pcur = x->pbegin;
  x->plast = x->pbegin + x->count;
  stop->pcur = stop->pbegin;
  stop->plast = stop->pbegin + stop->count;

  if (stop->pcur < stop->plast)
    {
      while (x->pcur < x->plast)
        {
          while (stop->pcur < stop->plast && stop->pcur->url_id < x->pcur->url_id)
            stop->pcur++;
...
static void
printBoolRes (DPS_AGENT *query, DPS_STACK_ITEM *res)
{
  size_t i;
  ;
...
static int
perform (DPS_AGENT *query, DPS_RESULT *Res, DPS_BOOLSTACK *s, int com)
{
                    ...
                    if ((Res->items[tt].order_origin & DPS_WORD_ORIGIN_STOP) == 0)
                      {
                        ...
                        gap_ahead[nonstop_words] = (tt == res.order_to) ? 0 : 1;
                        {
                          ...
                          for (zz = tt + 1; zz <= res.order_to; zz++)
                            {
                              if (Res->items[zz].order_origin & DPS_WORD_ORIGIN_STOP)
                                gap_ahead[nonstop_words]++;
                              else
                                break;
                            }
                        }
                  ...
                  while (res.pcur < res.plast)
                    {
                      ...
                      for (tt = 0; tt < nwords - nonstop_words; tt++)
                        pos_real[tt] = 0;
                      for (tt = 0; (tt < nonstop_words) && (res.pcur < res.plast) && (res.pcur->url_id == curlid); tt++)
                        {
                          ...
                          while ((res.pcur < res.plast) && (pos_real[p_ins] == DPS_WRDPOS (res.pcur->coord)))
                            res.pcur++;
                          p_ins++;
                          p_cmp++;
                          p_ins %= nwords;
...
/*			  printBoolRes(query, x2);*/
          ...
          if (DPS_OK != proceedOR (query, &res, x1, x2))
            return DPS_ERROR;
          DpsStackItemFree (x1);
          DpsStackItemFree (x2);
          res.count = res.pcur - res.pbegin;
          {
            ...
            else if (((x2origin & (DPS_WORD_ORIGIN_STOP | DPS_WORD_ORIGIN_QUERY)) == (DPS_WORD_ORIGIN_STOP | DPS_WORD_ORIGIN_QUERY)))
              {
                res.origin = x2origin;
              }
            else if (((x1origin & DPS_WORD_ORIGIN_STOP) && (x2origin & DPS_WORD_ORIGIN_STOP)) ||
                     ((res.count == 0) &&
      ...
      if (x2 == NULL || x1 == NULL)
      ...
      else
        {
          ...
          if ((x1->origin & DPS_WORD_ORIGIN_STOP) && (x2->origin & DPS_WORD_ORIGIN_STOP))
            ...
            {
              if (DPS_OK != proceedSTOP (query, &res, x1, x2))
                return DPS_ERROR;
            }
          else if (x1->origin & DPS_WORD_ORIGIN_STOP)
            {
              if (DPS_OK != proceedSTOP (query, &res, x2, x1))
                return DPS_ERROR;
            }
...
/*			    printBoolRes(query, x2);*/
              ...
              while (x1->pcur < x1->plast && x2->pcur < x2->plast)
                {
                  while ((x2->pcur < x2->plast) && (x2->pcur->url_id < x1->pcur->url_id))
                    x2->pcur++;
                  if (x2->pcur >= x2->plast)
                    break;
                  if (x2->pcur->url_id == x1->pcur->url_id)
                    {
                          ...
                          if (x1->pchecked->coord <= x2->pchecked->coord)
                            {
                              ...
                              if (x1->pchecked >= x1->plast || x1->pchecked->url_id != curlid)
                                break;
                            }
                          else
                            {
                              ...
                              if (x2->pchecked >= x2->plast || x2->pchecked->url_id != curlid)
                                break;
                            }
                          if (pos1 > pos2)
                            {
                      ...
                      if (x2->cmd & DPS_STACK_WORD_NOT || x1->cmd & DPS_STACK_WORD_NOT)
                        found = !found;
                      if (found)
                        {
                              ...
                              if (x1->pcur->coord <= x2->pcur->coord)
                                {
                                  *res.pcur = *x1->pcur;
                                  res.pcur++;
                                  x1->pcur++;
                                  if (x1->pcur >= x1->plast || x1->pcur->url_id != curlid)
                                    break;
                                }
                              else
                                {
                                  *res.pcur = *x2->pcur;
                                  res.pcur++;
                                  x2->pcur++;
                                  if (x2->pcur >= x2->plast || x2->pcur->url_id != curlid)
                                    break;
                                }
                            }
...
void
DpsWWLBoolItems (DPS_RESULT *Res)
{
  DPS_WIDEWORD Word;
...
int
DpsCalcBoolItems (DPS_AGENT *query, DPS_RESULT *Res)
{
  ...
  for (i = 0; i < nitems; i++)
    {
      ...
      if ((items[i].pbegin == NULL) && ((items[i].origin & DPS_WORD_ORIGIN_STOP) == 0))
        {
          for (j = 0; j < i; j++)
            if (items[j].crcword == items[i].crcword && (items[j].pbegin != NULL) && (items[j].secno == 0 || items[j].secno == items[i].secno))
              break;
          if (j < i)
            {
  ...
  for (i = 0; i < nitems; i++)
    {
      ...
      switch (c = items[i].cmd)
        ...
        case DPS_STACK_PHRASE_RIGHT:
          ...
          if (TOPCMD (s) == DPS_STACK_PHRASE_LEFT)
            perform (query, Res, s, POPCMD (s));
#ifdef DEBUG_BOOL
