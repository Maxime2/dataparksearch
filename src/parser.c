...
DpsParserAdd (DPS_PARSERLIST *List, DPS_PARSER *P)
{
...
DpsParserListFree (DPS_PARSERLIST *List)
{
  size_t i;
...
/* Parser1: from STDIN to STDOUT */
static char *
parse1 (DPS_AGENT *Agent, DPS_DOCUMENT *Doc, const char *url, const char *cmd)
{
  int wr[2];
  int rd[2];
  int rc;
          ...
          while ((res = write (wr[1], Doc->Buf.content + bytes_written, Doc->Buf.size - gap - bytes_written)) > 0)
            {
              ...
              if (bytes_written == Doc->Buf.size - gap)
                break;
            }
          close (wr[1]);
...
static char *
parse2 (DPS_AGENT *Agent, DPS_DOCUMENT *Doc, const char *url, const char *cmd)
{
...
static char *
parse3 (DPS_AGENT *Agent, DPS_DOCUMENT *Doc, const char *url, const char *cmd, const char *to_file)
{
  int fd;
  int rc;
  pid_t pid;
...
static char *
parse4 (DPS_AGENT *Agent, DPS_DOCUMENT *Doc, const char *url, const char *cmd, const char *to_file)
{
  size_t gap = Doc->Buf.content - Doc->Buf.buf;
  ssize_t rs;
  int wr[2], fd, rc;
...
static char *
parse_file (DPS_AGENT *Agent, DPS_PARSER *parser, DPS_DOCUMENT *Doc, const char *url)
{
  ...
  switch (parser_type)
    {
    case 1:
      result = parse1 (Agent, Doc, url, cmd);
      break;
    case 2:
      result = parse2 (Agent, Doc, url, cmd);
      break;
    case 3:
      result = parse3 (Agent, Doc, url, cmd, fnames[1]);
      break;
    case 4:
      result = parse4 (Agent, Doc, url, cmd, fnames[1]);
      break;
    }
...
__C_LINK DPS_PARSER *__DPSCALL
DpsParserFind (DPS_PARSERLIST *List, const char *mime_type)
{
  size_t i;
  for (i = 0; i < List->nparsers; i++)
    {
...
char *
DpsParserExec (DPS_AGENT *Agent, DPS_PARSER *P, DPS_DOCUMENT *Doc)
{
  char *result;
  if (P->cmd[0] == (char) '\0')
    return Doc->Buf.content;
