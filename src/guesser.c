...
DPS_LANG_ALIAS dps_lang_alias[] = {
  /* This table is ordered by the language name */
  { DPS_LANG_AA, "aa" },
  { DPS_LANG_AA, "aar" },
  { DPS_LANG_AB, "ab" },
  { DPS_LANG_AB, "abk" },
...
const char *
DpsLanguageCanonicalName (const char *name)
{
  int l, m, r, s;
  ...
  if (name != NULL)
    {
      ...
      while (l < r)
        {
          m = (l + r) / 2;
          if (strcasecmp (dps_lang_alias[m].name, name) < 0)
            l = m + 1;
          else
            r = m;
        }
...
static int
LangMapCmp (const DPS_LANGMAP *m1, const DPS_LANGMAP *m2)
{
...
static DPS_LANGMAP *
FindLangMap (DPS_LANGMAPLIST *L, const char *lang, const char *charset, const char *filename, const int addnew)
{
  DPS_LANGMAP *o = NULL;
  ssize_t i, l, r;
  register int c;
  ...
  for (i = 0; i <= DPS_LM_HASHMASK; i++)
    o->memb3[i].index = o->memb6[i].index = i;
...
DpsLoadLangMapFile (DPS_LANGMAPLIST *L, const char *filename)
{
  struct stat sb;
...
__C_LINK int __DPSCALL
DpsLoadLangMapList (DPS_LANGMAPLIST *L, const char *mapdir)
{
...
void
DpsLangMapListFree (DPS_LANGMAPLIST *List)
{
  size_t i;
...
void
DpsLangMapListSave (DPS_LANGMAPLIST *List)
{
  char time_str[128];
  size_t i, j, minv;
  FILE *out;
      ...
      if (Cmap->needsave)
        {
          if (Cmap->filename == NULL)
            ...
            {
              if ((out = fopen (Cmap->filename, "w")) == NULL)
                continue;
            }
...
static const char *
dps_next_char2map (register const char *p, register const char *end)
{
  for (; p < end; p++)
    {
...
void
DpsBuildLangMap (DPS_LANGMAP *map, const char *text, size_t textlen, size_t max_nbytes, int StrFlag)
{
  ...
  for (; p < end2; p = dps_next_char2map (++p, end2))
    {
      ...
      if ((max_nbytes > 0) && (map->nbytes + ngl > max_nbytes))
        break;
    }
  map->nbytes += ngl;
}
...
void
DpsBuildLangMap6 (DPS_LANGMAP *map, const char *text, size_t textlen, size_t max_nbytes, int StrFlag)
{
  const char *end2 = text + textlen;
  register const char *p;
  size_t ngl = 0;
...
int
DpsLMstatcmp (const void *i1, const void *i2)
{
...
int
DpsLMcmpCount (const void *i1, const void *i2)
{
...
*/
int
DpsLMcmpIndex (const void *v1, const void *v2)
{
...
void
DpsPrepareLangMap (DPS_LANGMAP *map)
{
  register int i;
  for (i = 0; i < DPS_LM_HASHMASK + 1; i++)
    map->memb3[i].index = map->memb6[i].index = i;
...
void
DpsPrepareLangMap6 (DPS_LANGMAP *map)
{
  register int i;
  for (i = 0; i < DPS_LM_HASHMASK + 1; i++)
    map->memb6[i].index = i;
...
void
DpsCheckLangMap (DPS_LANGMAP *map0, DPS_LANGMAP *map1, DPS_MAPSTAT *Stat, size_t InfMiss, size_t InfHits)
{
  register int i;
  DPS_LANGITEM *HIT;
...
#endif
      if (Stat->diff > InfMiss && Stat->hits > InfHits)
        break;
    }
}

void
DpsCheckLangMap6 (DPS_LANGMAP *map0, DPS_LANGMAP *map1, DPS_MAPSTAT *Stat, size_t InfMiss, size_t InfHits)
{
  register int i;
  DPS_LANGITEM *HIT;
...
int
DpsGuessCharSet (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, DPS_LANGMAPLIST *List)
{
  DPS_CHARSET *cs;
  ...
  */
      ...
      if (Indexer->Flags.nmaps && Doc->method != DPS_METHOD_DISALLOW)
        {
          ...
          for (i = 0; i <= DPS_LM_HASHMASK; i++)
            Indexer->LangMap->memb3[i].index = Indexer->LangMap->memb6[i].index = i;
          for (i = 0; i < Doc->TextList.nitems; i++)
            {
              ...
              if (Indexer->Flags.GuesserBytes > 0 && Indexer->LangMap->nbytes >= Indexer->Flags.GuesserBytes)
                break;
            }
        }
      ...
      for (i = 0; i < List->nmaps; i++)
        {
          ...
          if (mapstat[i].diff < InfMiss)
            {
              InfMiss = mapstat[i].diff;
              InfHits = mapstat[i].hits;
            }
        }

      /* Sort statistics in quality order */
      if (List->nmaps > 1)
        DpsPreSort (mapstat, List->nmaps, sizeof (DPS_MAPSTAT), &DpsLMstatcmp);
        ...
        for (i = 0; i < List->nmaps; i++)
          {
            ...
            if (*lang == '\0' && mapstat[i].map->lang)
              {
                if (
                    strcasecmp (mapstat[i].map->charset, charset) && strcasecmp (mapstat[i].map->charset, DPS_NULL2EMPTY (server_charset)) && strcasecmp (mapstat[i].map->charset, DPS_NULL2EMPTY (meta_charset)))
                  continue;
            ...
            if (*charset == '\0' && mapstat[i].map->charset)
              {
                if (!strcasecmp (mapstat[i].map->charset, DPS_NULL2EMPTY (server_charset)))
                  {
            ...
            if (*charset && *lang)
              break;
            if ((i > 50) && (mapstat[i].miss > mapstat[0].miss + mapstat[0].miss / 10))
              break;
          }
  ...
  if (*DPS_NULL2EMPTY (charset) == '\0')
    {
      if (use_meta && (*meta_charset != '\0'))
        DpsVarListReplaceStr (&Doc->Sections, "Charset", charset = meta_charset);
      else if (*server_charset != '\0')
        DpsVarListReplaceStr (&Doc->Sections, "Charset", charset = server_charset);
      else
        DpsVarListReplaceStr (&Doc->Sections, "Charset", charset = "iso8859-1");
    }
  if (*DPS_NULL2EMPTY (lang) == '\0')
    {
      if (use_meta && (*meta_lang != '\0'))
        DpsVarListReplaceStr (&Doc->Sections, "Content-Language", lang = meta_lang);
      else if (*server_lang != '\0')
        DpsVarListReplaceStr (&Doc->Sections, "Content-Language", lang = server_lang);
      else
  ...
  u = u && (forte_lang || (!strcasecmp (DpsLanguageCanonicalName (lang), DPS_NULL2EMPTY (lang0)) && ((*server_lang != '\0' && (!strcasecmp (DpsLanguageCanonicalName (server_lang), DPS_NULL2EMPTY (lang0)))) || (*meta_lang != '\0' && (!strcasecmp (DpsLanguageCanonicalName (meta_lang), DPS_NULL2EMPTY (lang0)))))));
  u = u && (forte_charset || (!strcasecmp (DpsLanguageCanonicalName (charset), DPS_NULL2EMPTY (charset0)) && ((*server_charset != '\0' && (!strcasecmp (DpsLanguageCanonicalName (server_charset), DPS_NULL2EMPTY (charset0)))) || (*meta_charset != '\0' && (!strcasecmp (DpsLanguageCanonicalName (meta_charset), DPS_NULL2EMPTY (charset0)))))));
