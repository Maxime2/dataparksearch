...
static int
SentCmp (const DPS_SENTENCE *s1, const DPS_SENTENCE *s2)
{
...
static int
SentOrderCmp (const DPS_SENTENCE *s1, const DPS_SENTENCE *s2)
{
...
int
DpsSEAMake (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, DPS_DSTR *excerpt, const char *content_lang, size_t *indexed_size, size_t *indexed_limit, size_t max_word_len, size_t min_word_len, int crossec, int seasec
#ifdef HAVE_ASPELL
            ,
            int have_speller,
...
)
{
  ...
  if ((Sec = DpsVarListFind (&Doc->Sections, "sea")))
    { /* set SEA section to NULL */
      DPS_FREE (Sec->val);
          ...
          if (j)
            {
              if (List.nitems < Indexer->Flags.SEASentences)
              ...
              else if (sent_len > min_len)
                {
                  ...
                  DPS_FREE (sentence);
                  min_len = List.Sent[0].len;
                  min_pos = 0;
                  for (i = 1; i < List.nitems; i++)
                    if (List.Sent[i].len < min_len)
                      {
                        min_len = List.Sent[i].len;
                        min_pos = i;
                      }
                }
            }
      ...
      for (l = 0; l < List.nitems; l++)
        {
          ...
          w = f (w);
          if (w < LOW_BORDER_EPS2)
            w = LOW_BORDER_EPS2;
          else if (w > HI_BORDER_EPS2)
            w = HI_BORDER_EPS2;
          List.Sent[l].di = w;
        }
