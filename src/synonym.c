...
void
DpsSynonymListInit (DPS_SYNONYMLIST *List)
{
  bzero ((void *) List, sizeof (*List));
}

__C_LINK int __DPSCALL
DpsSynonymListLoad (DPS_ENV *Env, const char *filename)
{
  struct stat sb;
  ...
  while (str != NULL)
    {
      if (str[0] == '#' || str[0] == ' ' || str[0] == HT_CHAR || str[0] == CR_CHAR || str[0] == NL_CHAR)
        goto loop_continue;
      ...
      else if (!strncasecmp (str, "Language:", 9))
        {
          char *lasttok;
          char *l;
      ...
      else if (!strncasecmp (str, "Thesaurus:", 10))
        {
          char *lasttok;
...
void
DpsSynonymListFree (DPS_SYNONYMLIST *List)
{
  size_t i;
...
static int
cmpsyn (const void *v1, const void *v2)
{
...
static int
cmpsynback (const void *v1, const void *v2)
{
  const DPS_SYNONYM **s1 = (const DPS_SYNONYM **) v1;
  const DPS_SYNONYM **s2 = (const DPS_SYNONYM **) v2;
...
DpsSynonymListSort (DPS_SYNONYMLIST *List)
{
...
DPS_WIDEWORDLIST *
DpsSynonymListFind (const DPS_SYNONYMLIST *List, DPS_WIDEWORD *wword)
{
              ...
              for (first = res; first >= List->Synonym; first--)
                {
                  if (DpsUniStrCmp (Res->Word[i].uword, first->p.uword))
                    ...
                    {
                      if ((Res->Word[i].count != 0) && (first->p.count != Res->Word[i].count))
                        continue;
                      first->s.order = wword->order;
                      first->s.origin = DPS_WORD_ORIGIN_SYNONYM;
                      DpsWideWordListAdd (Res, &first->s, DPS_WWL_LOOSE);
              ...
              for (last = res + 1; last < List->Synonym + List->nsynonyms; last++)
                {
                  if (DpsUniStrCmp (Res->Word[i].uword, last->p.uword))
                    ...
                    {
                      if ((Res->Word[i].count != 0) && (last->p.count != Res->Word[i].count))
                        continue;
                      last->s.order = wword->order;
                      last->s.origin = DPS_WORD_ORIGIN_SYNONYM;
                      DpsWideWordListAdd (Res, &last->s, DPS_WWL_LOOSE);
              ...
              for (pfirst = pres; pfirst >= List->Back; pfirst--)
                {
                  if (DpsUniStrCmp (syn.s.uword, (*pfirst)->s.uword))
                    ...
                    {
                      if ((Res->Word[i].count != 0) && ((*pfirst)->s.count != Res->Word[i].count))
                        continue;
              ...
              for (plast = pres + 1; plast < List->Back + List->nsynonyms; plast++)
                {
                  if (DpsUniStrCmp (syn.s.uword, (*plast)->s.uword))
                    ...
                    {
                      if ((Res->Word[i].count != 0) && ((*plast)->s.count != Res->Word[i].count))
                        continue;
