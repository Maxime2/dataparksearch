...
*/
int
DpsCmpUrlid (const void *v1, const void *v2)
{
...
static int
DpsCmpUrlid0 (const void *v1, const void *v2)
{
...
static int
DpsCmpPattern_U (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  if (L->Coords[i].url_id > L->Coords[j].url_id)
    return -1;
  if (L->Coords[i].url_id < L->Coords[j].url_id)
    return 1;
  return 0;
...
static int
DpsCmpPattern_U_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                   DPS_URL_CRD_DB *Coords,
#else
                   DPS_URL_CRD *Coords,
                   ...
                   const char *pattern)
{
  if (L->Coords[i].url_id > Coords->url_id)
    return -1;
  if (L->Coords[i].url_id < Coords->url_id)
    return 1;
  return 0;
...
static int
DpsCmpPattern_DRP (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  if (L->Data[i].last_mod_time > L->Data[j].last_mod_time)
    return -1;
  if (L->Data[i].last_mod_time < L->Data[j].last_mod_time)
    return 1;
  if (L->Coords[i].coord > L->Coords[j].coord)
    return -1;
  if (L->Coords[i].coord < L->Coords[j].coord)
    return 1;
  if (L->Data[i].pop_rank > L->Data[j].pop_rank)
    return -1;
  if (L->Data[i].pop_rank < L->Data[j].pop_rank)
    return 1;
  return 0;
...
static int
DpsCmpPattern_DRP_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                     DPS_URL_CRD_DB *Coords,
#else
                     DPS_URL_CRD *Coords,
                     ...
                     const char *pattern)
{
  if (L->Data[i].last_mod_time > Data->last_mod_time)
    return -1;
  if (L->Data[i].last_mod_time < Data->last_mod_time)
    return 1;
  if (L->Coords[i].coord > Coords->coord)
    return -1;
  if (L->Coords[i].coord < Coords->coord)
    return 1;
  if (L->Data[i].pop_rank > Data->pop_rank)
    return -1;
  if (L->Data[i].pop_rank < Data->pop_rank)
    return 1;
  return 0;
...
static int
DpsCmpPattern_PRD (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  if (L->Data[i].pop_rank > L->Data[j].pop_rank)
    return -1;
  if (L->Data[i].pop_rank < L->Data[j].pop_rank)
...
static int
DpsCmpPattern_PRD_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                     DPS_URL_CRD_DB *Coords,
#else
                     DPS_URL_CRD *Coords,
...
static int
DpsCmpPattern_RP (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  if (L->Coords[i].coord > L->Coords[j].coord)
    return -1;
  if (L->Coords[i].coord < L->Coords[j].coord)
...
static int
DpsCmpPattern_RP_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                    DPS_URL_CRD_DB *Coords,
#else
                    DPS_URL_CRD *Coords,
...
static int
DpsCmpPattern_RPD (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  if (L->Coords[i].coord > L->Coords[j].coord)
    return -1;
  if (L->Coords[i].coord < L->Coords[j].coord)
...
static int
DpsCmpPattern_RPD_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                     DPS_URL_CRD_DB *Coords,
#else
                     DPS_URL_CRD *Coords,
...
static int
DpsCmpPattern_IRPD (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  double m1 = L->Data[i].pop_rank, m2 = L->Data[j].pop_rank;
  m1 *= (double) L->Coords[i].coord;
  m2 *= (double) L->Coords[j].coord;
  if (m1 > m2)
    return -1;
  if (m1 < m2)
    return 1;
...
static int
DpsCmpPattern_IRPD_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                      DPS_URL_CRD_DB *Coords,
#else
                      DPS_URL_CRD *Coords,
...
static inline int
DpsCmpPattern_full (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{
  double m1, m2;
  ...
  for (; *pattern != '\0'; pattern++)
    {
      switch (*pattern)
        ...
        case 'd':
          ...
          break;
        case 'I':
          m1 = L->Data[i].pop_rank;
          m2 = L->Data[j].pop_rank;
          m1 *= (double) L->Coords[i].coord;
          m2 *= (double) L->Coords[j].coord;
          if (m1 > m2)
            return -1;
        ...
        case 'i':
          ...
          break;
        case 'A':
          m1 = L->Data[i].pop_rank * 1000.0;
          m2 = L->Data[j].pop_rank * 1000.0;
          m1 += (double) L->Coords[i].coord;
          m2 += (double) L->Coords[j].coord;
          if (m1 > m2)
            return -1;
...
static inline int
DpsCmpPattern_full_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                      DPS_URL_CRD_DB *Coords,
#else
                      DPS_URL_CRD *Coords,
...
static int
DpsCmpSiteid (DPS_URLCRDLIST *L, size_t i, size_t j, const char *pattern)
{

  if (L->Data[i].site_id < L->Data[j].site_id)
    return -1;
  if (L->Data[i].site_id > L->Data[j].site_id)
    return 1;
...
static int
DpsCmpSiteid_T (DPS_URLCRDLIST *L, size_t i,
#ifdef WITH_MULTIDBADDR
                DPS_URL_CRD_DB *Coords,
#else
                DPS_URL_CRD *Coords,
                ...
                const char *pattern)
{
  ...
  if (L->Data[i].site_id < Data->site_id)
    return -1;
  if (L->Data[i].site_id > Data->site_id)
    return 1;
...
*/
void
DpsSortSearchWordsByURL (DPS_URL_CRD_DB *wrd, size_t num)
{
  if (wrd != NULL && num > 1)
    DpsPreSort ((void *) wrd, num, sizeof (*wrd), (qsort_cmp) DpsCmpUrlid);
  return;
}

void
DpsSortSearchWordsByURL0 (DPS_URL_CRD *wrd, size_t num)
{
  if (wrd != NULL && num > 1)
    DpsPreSort ((void *) wrd, num, sizeof (*wrd), (qsort_cmp) DpsCmpUrlid0);
  return;
}

#define med3a(func, a, b, c, L, pattern) ((func (L, a, b, pattern) < 0) ? ((func (L, b, c, pattern) < 0) ? (b) : ((func (L, a, c, pattern) < 0) ? (c) : (a))) \
                                                                        : ((func (L, b, c, pattern) > 0) ? (b) : ((func (L, a, c, pattern) <= 0) ? (a) : (c))))

#define med3b(func, a, b, c, L, pattern) ((func (L, a, b, pattern) < 0) ? ((func (L, b, c, pattern) <= 0) ? (b) : ((func (L, a, c, pattern) < 0) ? (c) : (a))) \
                                                                        : ((func (L, b, c, pattern) >= 0) ? (b) : ((func (L, a, c, pattern) < 0) ? (a) : (c))))

#define med3c(func, a, b, c, L, pattern) ((func (L, a, b, pattern) < 0) ? ((func (L, b, c, pattern) < 0) ? (b) : ((func (L, a, c, pattern) <= 0) ? (c) : (a))) \
                                                                        : ((func (L, b, c, pattern) > 0) ? (b) : ((func (L, a, c, pattern) < 0) ? (a) : (c))))
...
static size_t
DpsPartitionSearchWordsBySite (DPS_RESULT *Res, DPS_URLCRDLIST *L, size_t p, size_t r, const char *pattern, int merge)
{
  DPS_URL_CRD_DB Crd;
  DPS_URLDATA Dat;
#ifdef WITH_REL_TRACK
...
static void *
DpsQsortSearchWordsBySite (void *arg)
{
  DPS_SORT_PARAM PP;
...
static void
DpsHeapSortSearchWordsBySite (void *arg)
{
  DPS_URLDATA Dat;
  DPS_SORT_PARAM *P = (DPS_SORT_PARAM *) arg;
...
void
DpsSortSearchWordsBySite (DPS_RESULT *Res, DPS_URLCRDLIST *L, size_t num, const char *pattern)
{
...
static size_t
DpsPartitionSearchWordsByPattern (DPS_RESULT *Res, DPS_URLCRDLIST *L, size_t p, size_t r, const char *pattern)
{
  size_t i = p, j = r, m, pl, pm, pn, d;
  size_t Cnt = 1;
  DPS_URL_CRD_DB Crd;
...
static void *
DpsQsortSearchWordsByPattern (void *arg)
{
  DPS_SORT_PARAM PP;
  DPS_SORT_PARAM *P = (DPS_SORT_PARAM *) arg;
  size_t i, j;
...
static void
DpsHeapSortSearchWordsByPattern (void *arg)
{
  DPS_URLDATA Dat;
  DPS_SORT_PARAM *P = (DPS_SORT_PARAM *) arg;
  DPS_URLCRDLIST *L = P->L;
...
void
DpsSortSearchWordsByPattern (DPS_RESULT *Res, DPS_URLCRDLIST *L, size_t num, const char *pattern)
{
...
static int
DpsExpandWord (DPS_AGENT *query, DPS_RESULT *Res, DPS_WIDEWORD *OWord, DPS_PREPARE_STATE *state)
{
  ...
  if (state->sp && !(Origin & DPS_WORD_ORIGIN_SPELL) && (forms = DpsAllForms (query, OWord)))
    {
      ...
      for (frm = 0; frm < forms->nwords; frm++)
        {
          if (DpsUniStrCmp (OWord->uword, forms->Word[frm].uword) == 0)
            continue;
...
int
dps_need2segment (dpsunicode_t *uwrd)
{
  dpsunicode_t u;
  for (; (u = *uwrd); uwrd++)
    {
      if (u >= 0xE01 && u <= 0xE5B)
        return 1; /* THAI */
      if (u == 0x321D || u == 0x321E || u == 0x327C || u == 0x327D || u == 0x327F)
        return 1; /* KOREAN */
      if (u >= 0x1100 && u <= 0x11F9)
        return 1; /* HANGUL */
      if (u == 0x302E || u == 0x302F)
        return 1;
      if ((u >= 0xAC00) && (u <= 0xD7A3))
        return 1;
      if (u >= 0x3131 && u <= 0x318E)
        return 1;
      if ((u >= 0x3200) && (u <= 0x321C))
        return 1;
      if (u >= 0x3260 && u <= 0x327F)
        return 1;
      if (u >= 0xFFA0 && u <= 0xFFDC)
        return 1;
      if (u >= 0x3031 && u <= 0x3035)
        return 1; /* KANA */
      if (u >= 0x3099 && u <= 0x309C)
        return 1;
      if (u >= 0x30A0 && u <= 0x30FF)
        return 1;
      if (u >= 0x31F0 && u <= 0x31FF)
        return 1;
      if (u >= 0x32D0 && u <= 0x32FF)
        return 1;
      if (u >= 0xFF65 && u <= 0xFF9F)
        return 1;
      if (u >= 0x2F00 && u <= 0x2F95)
        return 1; /* KANGXI */
      if (u >= 0x3041 && u <= 0x30A0)
        return 1; /* HIRAGANA */
      if (u == 0x30FC || u == 0xFF70)
        return 1;
      if ((u >= 0x2E80) && (u <= 0x2EF3))
        return 1; /* CJK */
      if ((u >= 0x31C0) && (u <= 0x31CF))
        return 1;
      if ((u >= 0xF900) && (u <= 0xFAD9))
        return 1;
      if ((u >= 0x2F800) && (u <= 0x2FA1D))
        return 1;
      if ((u >= 0x3400) && (u <= 0x4DB5))
        return 1;
      if ((u >= 0x4E00) && (u <= 0x9FBB))
        return 1;
      if ((u >= 0x20000) && (u <= 0x2A6D6))
        return 1;
    }
  return 0;
}
...
static dpsunicode_t *
DpsVQLparse (dpsunicode_t *query)
{
  int cmd_stack[1024];
  ...
  while (*s)
    {
      if (*s == '<')
          ...
          for (n = 0; VQLcmd[n].len; n++)
            {
              if (DpsUniStrNCaseCmp (s, VQLcmd[n].vql_cmd, VQLcmd[n].len) == 0)
                {
                  if (n)
                    cmd_stack[++p_cmd] = cur_cmd = VQLcmd[n].cmd;
                  break;
                }
            }
          while (*s && (*s != '>'))
        ...
        {
          s++;
          while ((p_cmd >= 0) && (cmd_stack[p_cmd] != DPS_STACK_LEFT))
            p_cmd--;
          if (p_cmd >= 0)
            p_cmd--;
          if ((p_cmd >= 0) && cmd_stack[p_cmd] == DPS_STACK_PHRASE_LEFT)
        ...
        {
          s++;
          if ((p_cmd < 0) || (cmd_stack[p_cmd] == DPS_STACK_PHRASE_LEFT) || ((p_cmd > 0) && (cmd_stack[p_cmd] == DPS_STACK_LEFT) && (cmd_stack[p_cmd - 1] == DPS_STACK_PHRASE_LEFT)))
            {
...
int
DpsPrepare (DPS_AGENT *query, DPS_RESULT *Res)
{
  ...
  if (state.qlang == NULL || *(state.qlang) == '\0')
    ...
    rlang = DpsLanguageCanonicalName (state.qlang);
  if (rlang == NULL)
    rlang = "en";
  ...
  if (*txt == '\0')
    { /* Empty query, trying to convert VQL */
  ...
  switch (search_mode)
    {
    case DPS_MODE_ANY:
      add_cmd = DPS_STACK_OR;
      break;
    case DPS_MODE_BOOL:
    case DPS_MODE_ALL:
      add_cmd = DPS_STACK_AND;
      break;
    case DPS_MODE_NEAR:
      add_cmd = DPS_STACK_NEAR;
      break;
    }
  ...
  switch (browser_cs->family)
    ...
    case DPS_CHARSET_CHINESE_TRADITIONAL:
      lang = "zh";
      break;
    case DPS_CHARSET_JAPANESE:
      lang = "ja";
      break;
    case DPS_CHARSET_THAI:
      lang = "th";
      break;
    case DPS_CHARSET_KOREAN:
      lang = "ko";
      break;
    case DPS_CHARSET_UNICODE:
      lang = (state.qlang == NULL) ? "" : state.qlang;
      break;
    default:
      lang = (state.qlang == NULL) ? "" : state.qlang;
    }
          ...
          for (i = 0; i < ULen; i++)
            {
              switch (uwrd[i])
                ...
                case '&':
                case '+':
                  cur_cmd = DPS_STACK_AND;
                  notfirstword = 0;
                  break;
                case '|':
                  cur_cmd = DPS_STACK_OR;
                  notfirstword = 0;
                  break;
                case '~':
                  cur_cmd = DPS_STACK_NOT;
                  notfirstword = 0;
                  break;
                case '(':
                  cur_cmd = DPS_STACK_LEFT;
                  notfirstword = 0;
      ...
      else
        {
          ...
          if (!(state.nphrasecmd & 1) && (strncasecmp (clex, "allin", 5) == 0))
          ...
          else
            {
              ...
              while (seg_lex)
                {
                  if (DPS_UNI_CTYPECLASS (seg_ctype) != DPS_UNI_BUKVA)
                    goto seg_next;
...
#ifdef HAVE_ASPELL
                  if (use_aspellext && seg_have_bukva_forte && seg_wlen > 2 && query->naspell < DPS_DEFAULT_MAX_ASPELL && (DpsUniStrChr (uwrd, (dpsunicode_t) '&') == NULL) /* aspell trap workaround */
                  )
                    {
                      toadd = 1;
                      ...
                      if (ii == 0)
                        {
                          ...
                          if (query->aspell_pid[query->naspell] > 0)
                          ...
                          else
                            {
                              ...
                              init_signals ();
                              close (0);
                              close (1); /* close STDIN and STDOUT */
                              close (rd[0]);
                          ...
                          if (/*!(state.nphrasecmd & 1)&&-*/ (first = DpsAcronymListFind (&query->Conf->Acronyms, &OWord, &last)) != NULL)
                            {
                              while (first <= last)
                                {
                                  if ((state.nphrasecmd & 1) && (first->unroll.nwords > 1))
                                    {
                                      first++;
                                      continue;
                                    }
      ...
      if ((Res->nitems > 0) && (Res->items[Res->nitems - 1].cmd == DPS_STACK_PHRASE_LEFT))
        {
          Res->nitems--;
          Res->ncmds--;
          while (Res->nitems > 0 && ((Res->items[Res->nitems - 1].cmd == DPS_STACK_AND) || (Res->items[Res->nitems - 1].cmd == DPS_STACK_OR) || (Res->items[Res->nitems - 1].cmd == DPS_STACK_NOT)))
            {
              Res->nitems--;
              Res->ncmds--;
            }
...
static inline int
DpsBitsCount (dps_uint4 x)
{
  return DpsBitCntTable[x & 0xFF] + DpsBitCntTable[(x >> 8) & 0xFF] + DpsBitCntTable[(x >> 16) & 0xFF] + DpsBitCntTable[(x >> 24) & 0xFF];
}

static inline dps_uint4
DpsCalcCosineWeightFull (dps_uint4 *R, double x, double xy, dps_uint4 *D, size_t phr_n
#ifdef WITH_REL_TRACK
                         ,
                         double *D_y
#endif
)
{
  ...
  if (D[DPS_N_PHRASE] == 0)
    ...
    y_phrase = 100 / D[DPS_N_PHRASE];
  if (D[DPS_N_EXACT] == 0)
    y_exact = 10000;
  else
    y_exact = 100 / D[DPS_N_EXACT];
...
static inline dps_uint4
DpsCalcCosineWeightFast (dps_uint4 *R, double x, dps_uint4 *D, size_t nw
#ifdef WITH_REL_TRACK
                         ,
                         double *D_y,
                         double *D_xy
...
static inline dps_uint4
DpsCalcCosineWeightUltra (dps_uint4 *R, double x, double xy, dps_uint4 *D, size_t ns, size_t phr_n
#ifdef WITH_REL_TRACK
                          ,
                          double *D_y
#endif
...
static inline dps_uint4
DpsOriginIndex (int origin)
{
  if (origin & DPS_WORD_ORIGIN_SYNONYM)
    return 6;
  if (origin & DPS_WORD_ORIGIN_ASPELL)
    return 5;
  if (origin & DPS_WORD_ORIGIN_ACRONYM)
    return 4;
  if (origin & DPS_WORD_ORIGIN_SPELL)
    return 3;
  if (origin & DPS_WORD_ORIGIN_ACCENT)
    return 2;
  if (origin & DPS_WORD_ORIGIN_QUERY)
    return 1;
  return DPS_WORD_ORIGIN_MAX;
}

static inline dps_uint4
DpsOriginWeightFull (int origin)
{ /* Weight for origin can be from 1 to 15 */
  if (origin & DPS_WORD_ORIGIN_SYNONYM)
    return 0x01;
  if (origin & DPS_WORD_ORIGIN_ASPELL)
    return 0x02;
  ...
  if (origin & DPS_WORD_ORIGIN_QUERY)
    return 0x30; /*0x24;*/
  if (origin & DPS_WORD_ORIGIN_COMMON)
    return 0x3F;
  return 0;
}

static inline dps_uint4
DpsOriginWeightFast (int origin)
{ /* Weight for origin can be from 1 to 15 */
  if (origin & DPS_WORD_ORIGIN_SYNONYM)
    return 0x1;
  if (origin & DPS_WORD_ORIGIN_ASPELL)
...
static inline dps_uint4
DpsOriginWeightUltra (int origin)
{ /* Weight for origin can be from 1 to 15 */
  if (origin & DPS_WORD_ORIGIN_ASPELL)
    return 0x010;
  if (origin & DPS_WORD_ORIGIN_SYNONYM)
...
static void
DpsGroupByURLFull (DPS_AGENT *query, DPS_RESULT *Res)
{
  ...
  if (DPS_OK != DpsCalcBoolItems (query, Res))
    {
      TRACE_OUT (query);
      return;
    }
  if (!Res->CoordList.ncoords || Res->nitems == 0)
    {
      TRACE_OUT (query);
      return;
    }
  ...
  if ((count = (size_t *) DpsXmalloc (2 * count_size + 1)) == NULL)
    {
      TRACE_OUT (query);
      return;
    }
  ...
  if (wordorder == 0)
    {
      cur_order = 0;
      cur_sec = wordsec;
      if (Res->WWList.Word[wordnum].origin == DPS_WORD_ORIGIN_QUERY)
        cur_exact = 1;
      else
        cur_exact = 0;
    }
      ...
      if (Crd[j].url_id == Crd[i].url_id)
        {
          ...
          if (count[wordorder] == 0)
            D[DPS_N_FIRSTPOS] += wordpos;
#endif
#ifdef WITH_REL_DISTANCE
          ...
          if ((wordorder == cur_order + 1) && ((cur_order == (size_t) -1) || ((wordpos == prev_wordpos[prev_wordorder] + 1) && (wordsec == cur_sec))))
            {
              ...
              if (cur_order == 0)
                {
                  cur_sec = wordsec;
                  cur_exact = (w_origin == DPS_WORD_ORIGIN_QUERY);
                }
              else
                cur_exact *= (w_origin == DPS_WORD_ORIGIN_QUERY);
      ...
      else
        {
          ...
          for (tt = 0; tt <= Res->max_order_inquery; tt++)
            {
              xy += xy_o[tt];
#ifdef WITH_REL_WRDCOUNT
              if (Res->items[tt].order_origin & DPS_WORD_ORIGIN_STOP)
                continue;
          ...
          for (tt++; tt < Res->max_order_inquery + DPS_WORD_ORIGIN_MAX; tt++)
            {
              if (count[D[DPS_N_ORIGIN]] < count[tt])
                D[DPS_N_ORIGIN] = tt;
            }
...
static void
DpsGroupByURLFast (DPS_AGENT *query, DPS_RESULT *Res)
{
  size_t i, j = 0, D_size, R_size, phr_n;
  ...
  if ((count = (size_t *) DpsXmalloc (count_size + 1)) == NULL)
    {
      TRACE_OUT (query);
      return;
    }
...
/*	    D[DPS_N_POSITION] += wordpos;*/
          ...
          if ((prev_wordorder != (wordorder + 1) % n_order_inquery) && (wordorder != (prev_wordorder + 1) % n_order_inquery))
            D[DPS_N_DISTANCE] += DPS_ORDER_PENALTY;
#endif
          count[wordorder]++;
        }
      else
...
static void
DpsGroupByURLUltra (DPS_AGENT *query, DPS_RESULT *Res)
{
  size_t i, j = 0, D_size, R_size, phr_n;
  size_t *count, count_size;
  size_t wordsec, wordpos, prev_wordpos, wordnum, prev_wordnum, wordorder, prev_wordorder;
...
/*	    D[DPS_N_POSITION] /= phr_n;*/
          ...
          for (tt = 0; tt <= Res->max_order_inquery; tt++)
            {
              if (count[tt])
                cc += D[DPS_N_ADD + nsections + tt] / count[tt];
#ifdef WITH_REL_WRDCOUNT
              if (count[tt])
                sum += ((count[tt] > median) ? (count[tt] - median) : (median - count[tt]));
              else
...
void
DpsGroupByURL (DPS_AGENT *query, DPS_RESULT *Res)
{
...
void
DpsGroupBySite (DPS_AGENT *query, DPS_RESULT *Res)
{
  size_t i, j = 0, cnt = 1;
  urlid_t Doc_site;
  ...
  if (Res->PerSite == NULL)
    {
      merge = 0;
      if ((Res->PerSite = (size_t *) DpsMalloc (sizeof (*Res->PerSite) * Res->CoordList.ncoords + 1)) == NULL)
        return;
    }
...
int
DpsParseQueryString (DPS_AGENT *Agent, DPS_VARLIST *vars, char *query_string)
{
  char *tok, *lt;
  size_t len;
  ...
  while (tok)
    {
      ...
      else if (strcasecmp (tok, "EtcDir") == 0 || strcasecmp (tok, "VarDir") == 0)
        {
          /* do nothing, don't allow to override those variables from a query */
        }
      else
        {
          if (strncasecmp (tok, "ul", 2) == 0)
            DpsVarListAddStr (vars, tok, str);
          else
          ...
          if ((lim = DpsVarListFindStr (vars, str, NULL)))
            {
              ...
              if (!strcasecmp (lim, "category"))
                ...
                {
                  ltype = DPS_LIMTYPE_HOUR;
                  fname = str;
                }
              else if (!strcasecmp (lim, "minute"))
                {
                  ltype = DPS_LIMTYPE_MINUTE;
                  fname = str;
                }
              else if (!strcasecmp (lim, "hostname"))
                {
...
char *
DpsHlConvert (DPS_WIDEWORDLIST *List, const char *src, DPS_CONV *lc_uni, DPS_CONV *uni_bc, int NOprefixHL)
{
      ...
      if (List != NULL)
        {
          for (uw = 0; uw < List->nwords; uw++)
            {
              if (List->Word[uw].origin & DPS_WORD_ORIGIN_STOP)
                continue;
              slen = List->Word[uw].ulen;
              if (slen > flen)
                continue;
              if (NOprefixHL && (DpsUniCType (tok[slen]) <= DPS_UNI_BUKVA) && (tok[slen] != 0) && (tok[slen] >= 0x30))
                continue;
...
int
DpsConvert (DPS_ENV *Conf, DPS_VARLIST *Env_Vars, DPS_RESULT *Res, DPS_CHARSET *lcs, DPS_CHARSET *bcs)
{
  size_t i, r, len;
  ...
  for (i = 0; i < Res->num_rows; i++)
    {
      ...
      if ((!Conf->Flags.make_prefixes) && strncasecmp (doclang, "zh", 2) && strncasecmp (doclang, "th", 2) && strncasecmp (doclang, "ja", 2) && strncasecmp (doclang, "ko", 2))
        NOprefixHL = 1;
...
char *
DpsRemoveHiLightDup (const char *s)
{
...
int
DpsCatToTextBuf (DPS_CATEGORY *C, char *textbuf, size_t len)
{
  char *end;
  size_t i;
...
int
DpsCatFromTextBuf (DPS_CATEGORY *C, char *textbuf)
{
...
dpsunicode_t *
DpsUniSegment (DPS_AGENT *Indexer, dpsunicode_t *ustr, const char *lang)
{
  DPS_DSTR S;
  DPS_CHARSET *tis_cs;
  ...
  if (!eucjp_cs)
    eucjp_cs = DpsGetCharSet ("sys-int");
...
char *
DpsBuildPageURL (DPS_VARLIST *vars, char **dst)
{
...
void
DpsParseQStringUnescaped (DPS_VARLIST *vars, const char *qstring)
{
  char *tok;
      ...
      while (tok)
        {
          ...
          if (arg)
            *arg++ = '\0';
          *tok = '\0';
          if (strncasecmp (name, "ul", 2) == 0)
            DpsVarListAddStr (vars, name, arg ? arg : "");
          else
...
size_t
DpsRemoveNullSections (DPS_URL_CRD *words, size_t n, int *wf)
{
  size_t i, j = 0;
  for (i = 0; i < n; i++)
    {
      ...
      if (s == 0 || wf[s] > 0)
        words[j++] = words[i]; /* zero section number means it's came from a limit */
    }
  return j;
}

size_t
DpsRemoveNullSectionsDB (DPS_URL_CRD_DB *words, size_t n, int *wf, int secno)
{
  size_t i, j = 0;
  int s;
  if (secno == 0)
    {
      for (i = 0; i < n; i++)
        {
          if (wf[DPS_WRDSEC (words[i].coord)] > 0)
            words[j++] = words[i];
        }
    }
  else
