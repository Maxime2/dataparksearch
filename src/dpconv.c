...
cmpgrp (const void *v1, const void *v2)
{
  ...
  if ((res = strcasecmp (csgroup (c1), csgroup (c2))))
    return res;
...
main (int argc, char **argv)
{
  ...
  if ((from_buf = (char *) DpsMalloc (DPCONV_BUF_SIZE)) == NULL)
    {
      if (verbose)
        fprintf (stderr, "Can't alloc memory, %d bytes", DPCONV_BUF_SIZE);
      exit (1);
    }
  if ((to_buf = (char *) DpsMalloc (16 * DPCONV_BUF_SIZE)) == NULL)
    {
      if (verbose)
        fprintf (stderr, "Can't alloc memory, %d bytes", 16 * DPCONV_BUF_SIZE);
      exit (1);
    }
  if ((uni_buf = (char *) DpsMalloc (4 * DPCONV_BUF_SIZE * sizeof (int))) == NULL)
    {
      if (verbose)
        fprintf (stderr, "Can't alloc memory, %d bytes", (int) (4 * DPCONV_BUF_SIZE * sizeof (int)));
      exit (1);
    }
  ...
  while ((fgets (from_buf, DPCONV_BUF_SIZE, stdin)) != NULL)
    {
      ...
      if (NFC)
        uni_buf = (char *) DpsUniNormalizeNFC ((dpsunicode_t *) uni_buf, (dpsunicode_t *) uni_buf);
      if (NFD)
        uni_buf = (char *) DpsUniNormalizeNFD ((dpsunicode_t *) uni_buf, (dpsunicode_t *) uni_buf);
      ...
      if (fwrite (to_buf, uni_tc.obytes, 1, stdout) != 1)
        {
          ...
          if (verbose)
            fprintf (stderr, "An output error ocurred");
          exit (1);
        }
    }
  ...
  if (ferror (stdin))
    {
      if (verbose)
        fprintf (stderr, "An input error ocurred");
      exit (1);
    }
