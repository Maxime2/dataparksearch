...
void
DpsAcronymListInit (DPS_ACRONYMLIST *List)
{
  bzero ((void *) List, sizeof (*List));
}

int __DPSCALL
DpsAcronymListLoad (DPS_AGENT *query, const char *filename)
{
  DPS_MATCH Alias;
...
void
DpsAcronymListFree (DPS_ACRONYMLIST *List)
{
  size_t i;
...
static int
cmpacr (const void *v1, const void *v2)
{
...
DpsAcronymListSort (DPS_ACRONYMLIST *List)
{
...
DPS_ACRONYM *
DpsAcronymListFind (const DPS_ACRONYMLIST *List, DPS_WIDEWORD *wword, DPS_ACRONYM **store_last)
{
  ...
  if (last >= List->Acronym + List->nacronyms)
    last--;
  *store_last = last;
  return first;
}
