...
static const dpsunicode_t *
DpsUniRegTok (const dpsunicode_t *s, const dpsunicode_t **last)
{
  int skip;
...
int
DpsUniRegComp (DPS_UNIREG_EXP *reg, const dpsunicode_t *pattern)
{
...
int
DpsUniRegExec (const DPS_UNIREG_EXP *reg, const dpsunicode_t *string)
{
...
void
DpsUniRegFree (DPS_UNIREG_EXP *reg)
{
  size_t i;
...
void
DpsUniRegCompileAll (DPS_ENV *Conf)
{
  size_t i;
...
static int
cmpspellword (dpsunicode_t *w1, const dpsunicode_t *w2)
{
  ...
  if (lc == 0)
    {
        ...
        }
      {
        ...
        if (((const DPS_AFFIX *) s1)->type == 'p')
          ...
          {
            u1[((const DPS_AFFIX *) s1)->replen - 1] &= 255;
            u2[((const DPS_AFFIX *) s2)->replen - 1] &= 255;
...
static int
cmpquffix (const void *s1, const void *s2)
{
  int lc;
...
int
DpsSpellAdd (DPS_SPELLLIST *List, const dpsunicode_t *word, const char *flag, const char *lang)
{
...
DpsImportDictionary (DPS_AGENT *query, const char *lang, const char *charset, const char *filename, int skip_noflag, const char *first_letters)
{
  struct stat sb;
...
static DPS_SPELL **
DpsFindWord (DPS_AGENT *Indexer, const dpsunicode_t *p_word, const char *affixflag, DPS_PSPELL *PS, DPS_PSPELL *FZ)
{
          ...
          if ((FZ != NULL) && (PS->nspell == 0) && ((affixflag == NULL) || (strstr (SpellList->Spell[c].flag, affixflag) != NULL)))
            {
              ...
              for (z = 0; word[z] && SpellList->Spell[c].word[z] == word[z]; z++)
                ;
              if (z > 3)
                { /* FIXME: min. 4 letters from the end, - add config parameter for that */
                  if (z > FZ->nspell)
                    {
...
int
DpsAffixAdd (DPS_AFFIXLIST *List, const char *flag, const char *lang, const dpsunicode_t *mask, const dpsunicode_t *find, const dpsunicode_t *repl, int type)
{
...
int
DpsQuffixAdd (DPS_QUFFIXLIST *List, const char *flag, const char *lang, const dpsunicode_t *mask, const dpsunicode_t *find, const dpsunicode_t *repl)
{
...
__C_LINK int __DPSCALL
DpsImportAffixes (DPS_ENV *Conf, const char *lang, const char *charset, const char *filename)
{
  struct stat sb;
  char *str, *data = NULL, *cur_n = NULL;
  char flag[2] = "";
  ...
  while (str != NULL)
    {
      ...
      remove_spaces (mstr, repl);
      dps_strcpy (repl, mstr);
      remove_spaces (mstr, find);
      dps_strcpy (find, mstr);
      remove_spaces (mstr, mask);
      dps_strcpy (mask, mstr);
...
__C_LINK int __DPSCALL
DpsImportQuffixes (DPS_ENV *Conf, const char *lang, const char *charset, const char *filename)
{
  struct stat sb;
  char *str, *data = NULL, *cur_n = NULL;
  char flag[2] = "";
...
DpsSortDictionary (DPS_SPELLLIST *List)
{
  int j, CurLet = -1, Let;
  size_t i;
  char *CurLang = NULL;
  ...
  if (List->sorted)
    return;
  if (List->nspell > 1)
    DpsSort ((void *) List->Spell, List->nspell, sizeof (DPS_SPELL), cmpspell);
...
__C_LINK void __DPSCALL
DpsSortAffixes (DPS_AFFIXLIST *List, DPS_SPELLLIST *SL)
{
      ...
      if (Affix->type == 'p')
        {
          ...
          if (cl < 0)
            continue; /* we have affixes without spell for this lang */
...
void
DpsSortQuffixes (DPS_QUFFIXLIST *List, DPS_SPELLLIST *SL)
{
...
static void
CheckSuffix (const dpsunicode_t *word, size_t len, DPS_AFFIX *Affix, int *res, DPS_AGENT *Indexer, DPS_PSPELL *PS, DPS_PSPELL *FZ)
{
...
static int
CheckPrefix (const dpsunicode_t *word, DPS_AFFIX *Affix, DPS_AGENT *Indexer, int li, int pi, DPS_PSPELL *PS, DPS_PSPELL *FZ)
{
  dpsunicode_t newword[2 * MAXNORMLEN] = { 0 };
  int err, ls, rs, res;
  /*  int lres,rres;*/
...
__C_LINK DPS_SPELL **__DPSCALL
DpsNormalizeWord (DPS_AGENT *Indexer, DPS_WIDEWORD *wword, DPS_PSPELL *FZ)
{
  size_t len;
  DPS_SPELL **forms;
  DPS_SPELL **cur;
  ...
  len = DpsUniLen (uword);
  if (len < Indexer->WordParam.min_word_len || len > MAXNORMLEN || len > Indexer->WordParam.max_word_len)
    return (NULL);
  ...
  *cur = NULL;
  ...
  if (nlang == -1)
    {
      li_from = 0;
      li_to = Indexer->Conf->Spells.nLang;
    }
  else
    {
      li_from = nlang;
...
void
DpsSpellListFree (DPS_SPELLLIST *List)
{
  size_t i;
...
void
DpsAffixListFree (DPS_AFFIXLIST *List)
{
  size_t i;
...
void
DpsQuffixListFree (DPS_QUFFIXLIST *List)
{
  size_t i;
...
static void
DpsAllFormsWord (DPS_AGENT *Indexer, DPS_SPELL *word, DPS_WIDEWORDLIST *result, size_t order, size_t order_inquery)
{
  DPS_WIDEWORD w;
  size_t i;
  ...
  if (local_charset == NULL)
    return;
  if (NULL == (sys_int = DpsGetCharSet ("sys-int")))
    return;
  ...
  for (i = 0; i < naffixes; i++)
    {
      if ((word->flag != NULL) && (strcmp (word->lang, Affix[i].lang) == 0) && (strstr (word->flag, Affix[i].flag) != NULL))
        {
          if (Affix[i].compile)
            {
          ...
          if (err && (err = (Affix[i].type == 'p') ? (DpsUniStrNCaseCmp (r_word, Affix[i].find, Affix[i].findlen) == 0) : (DpsUniStrBNCmp (r_word, Affix[i].find, Affix[i].findlen) == 0)))
            {
...
static void
DpsQuffixWord (DPS_AGENT *Indexer, DPS_WIDEWORDLIST *result, DPS_SPELL *norm, DPS_WIDEWORD *wword)
{
  DPS_WIDEWORD w;
  ...
  if (NULL == (sys_int = DpsGetCharSet ("sys-int")))
    return;
  if ((PS.cur = (DPS_SPELL **) DpsXmalloc (MAX_NORM * sizeof (DPS_SPELL *))) == NULL)
    return;
  PS.nspell = 0;
  DpsConvInit (&fromuni, sys_int, local_charset, Indexer->Conf->CharsToEscape, DPS_RECODE_HTML);
  ...
  for (i = 0; i < nrecs; i++)
    {
      if (((norm->flag != NULL) && (strcmp (norm->lang, Quffix[i].lang) == 0) && (strstr (norm->flag, Quffix[i].flag) != NULL)) || ((norm->flag == NULL) && (strcmp (norm->lang, Quffix[i].lang) == 0) && (strchr (Quffix[i].flag, (int) '.') != NULL)))
        {
          if (Quffix[i].compile)
            {
...
__C_LINK DPS_WIDEWORDLIST *__DPSCALL
DpsAllForms (DPS_AGENT *Indexer, DPS_WIDEWORD *wword)
{
  DPS_SPELL **norm, **cur;
  ...
  if ((wword->ulen < Indexer->WordParam.min_word_len) || (wword->ulen == 1))
    return NULL;
  local_charset = Indexer->Conf->lcs;
  if (local_charset == NULL)
    return NULL;
  ...
  if (cur != NULL)
      ...
      while (*cur != NULL)
        {
          ...
          if (sy)
            syn = DpsSynonymListFind (&(Indexer->Conf->Synonyms), &w);
          ...
          if (sp)
            {
              DpsAllFormsWord (Indexer, *cur, result, wword->order, wword->order_inquery);
              if (wword->origin & DPS_WORD_ORIGIN_QUERY)
                DpsQuffixWord (Indexer, result, *cur, wword);
            }
          if (syn != NULL)
            {
              for (i = 0; i < syn->nwords; i++)
                  ...
                  if (DpsUniStrCmp (aw, (*cur)->word))
                    {
                      ...
                      if (sp)
                        {
                          DpsAllFormsWord (Indexer, &asp, result, wword->order, wword->order_inquery);
                          if (wword->origin & DPS_WORD_ORIGIN_QUERY)
                            DpsQuffixWord (Indexer, result, &asp, wword);
                        }
                    }
                  DPS_FREE (aw);
                  if (zz == 0)
  ...
  else if (/*FZ.nspell > 0 && */ FZ.nspell > Indexer->WordParam.min_word_len)
    {
      ...
      if (sp)
        {
          DpsAllFormsWord (Indexer, p_sp, result, wword->order, wword->order_inquery);
          if (wword->origin & DPS_WORD_ORIGIN_QUERY)
            DpsQuffixWord (Indexer, result, p_sp, wword);
        }
      if (syn != NULL)
        {
          for (i = 0; i < syn->nwords; i++)
  ...
  else
    {
      ...
      if (sy)
        syn = DpsSynonymListFind (&(Indexer->Conf->Synonyms), wword);
