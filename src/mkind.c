...
static int
cmp_ind8 (const DPS_UINT8URLID *c1, const DPS_UINT8URLID *c2)
{
  if (c1->hi < c2->hi)
    return -1;
  if (c1->hi > c2->hi)
...
static int
cmp_ind4 (const DPS_UINT4URLID *c1, const DPS_UINT4URLID *c2)
{
  if (c1->val < c2->val)
    return -1;
  if (c1->val > c2->val)
...
static void
ClearIndex4 (DPS_UINT4URLIDLIST *L)
{
  if (L->mapped)
    {
...
static void
ClearIndex8 (DPS_UINT8URLIDLIST *L)
{
  DPS_FREE (L->Item);
...
static int
MakeNestedIndex (DPS_AGENT *Indexer, DPS_UINT8URLIDLIST *L, const char *lim_name, DPS_DB *db)
{
  ...
  if (L->nitems > 1)
    DpsSort (L->Item, L->nitems, sizeof (DPS_UINT8URLID), (qsort_cmp) cmp_ind8);
...
static int
MakeLinearIndex (DPS_AGENT *Indexer, DPS_UINT4URLIDLIST *L, const char *lim_name, DPS_DB *db)
{
  DPS_ENV *Conf = Indexer->Conf;
  size_t k, prev;
  urlid_t *data = NULL;
  ...
  if (L->nitems > 1)
    DpsSort (L->Item, L->nitems, sizeof (DPS_UINT4URLID), (qsort_cmp) cmp_ind4);
...
static int
MakeLinearIndexLinks (DPS_AGENT *Indexer, const char *lim_name, DPS_DB *db)
{
  char fname[PATH_MAX];
  ...
  while (u)
    {
      ...
      if (DpsNeedLog (DPS_LOG_EXTRA))
        dps_setproctitle ("[%d] links data: %ld records processed", Indexer->handle, offset);
...
DpsCacheMakeIndexes (DPS_AGENT *Indexer, DPS_DB *db)
{
#ifdef HAVE_SQL
      ...
      if (!strncasecmp ("Limit-", v->Root[r].Var[i].name, 6))
        {
          ...
          if (!strcasecmp (ind, "category"))
          ...
          else if (!strcasecmp (ind, "since"))
            {
              ...
              /* To see the URL being indexed in "ps" output */
              if (DpsNeedLog (DPS_LOG_EXTRA))
                dps_setproctitle ("[%d] Since Time index creation", Indexer->handle);
              ...
              if (req != NULL)
                {
                  ...
                  if (!strcasecmp (ind, "hex8str"))
                  ...
                  else
                    {
                      ...
                      if (!strcasecmp (ind, "strcrc32"))
                        field_type = DPS_IFIELD_TYPE_STRCRC32;
                      else if (!strcasecmp (ind, "hour"))
                        field_type = DPS_IFIELD_TYPE_HOUR;
                      else if (!strcasecmp (ind, "minute"))
                        field_type = DPS_IFIELD_TYPE_MIN;
                      else if (!strcasecmp (ind, "hostname"))
                        field_type = DPS_IFIELD_TYPE_HOSTNAME;
                      else if (!strcasecmp (ind, "char2"))
                        field_type = DPS_IFIELD_TYPE_STR2CRC32;
                      else if (!strcasecmp (ind, "int"))
                        field_type = DPS_IFIELD_TYPE_INT;
