...
int
DpsCookiesAdd (DPS_AGENT *Indexer, const char *domain, const char *path, const char *name, const char *value, const char secure, dps_uint4 expires, const char from_config, int insert_flag)
{
#ifdef HAVE_SQL
...
int
DpsCookiesAddStr (DPS_AGENT *Indexer, DPS_URL *CurURL, char *cookie_str, int insert_flag)
{
  char *part, *lpart;
  char *name = NULL;
  char *value = NULL;
      ...
      if ((arg = strchr (part, '=')))
        {
          *arg++ = '\0';
          ...
          else if (!strcasecmp (part, "path"))
            {
              DpsFree (path);
              path = DpsStrdup (arg);
            }
          else if (!strcasecmp (part, "domain"))
            {
              DpsFree (orig_domain);
              orig_domain = domain = DpsStrdup (arg);
            }
          else if (!strcasecmp (part, "secure"))
            {
              secure = 'y';
            }
          else if (!strcasecmp (part, "expires"))
            {
...
void
DpsCookiesFree (DPS_COOKIES *Cookies)
{
  size_t i;
  DPS_COOKIE *Coo;
...
void
DpsCookiesFind (DPS_AGENT *Indexer, DPS_SERVER *Server, DPS_DOCUMENT *Doc, const char *hostinfo)
{
#ifdef HAVE_SQL
  DPS_DSTR cookie;
  DPS_COOKIES *Cookies = &Indexer->Cookies;
  ...
  for (i = 0; i < Cookies->ncookies; i++)
    {
      ...
      if (slen > blen)
        continue;
      if (Coo->secure == 'y' && strcasecmp (Doc->CurURL.schema, "https"))
        continue;
      if (strncasecmp (Coo->path, Doc->CurURL.path, dps_strlen (Coo->path)))
        continue;
      if (strcasecmp (Coo->domain, hostinfo + (blen - slen)))
        continue;
      if (Coo->from_config != 1)
        have_no_cookies = 0;
      if (Coo->name[0] == '\0' && Coo->value[0] == '\0')
        continue;
      if (cookie.data_size)
              ...
              for (i = 0; i < rows; i++)
                {
                  ...
                  if (*DpsSQLValue (&Res, i, 3) == 'y' && strcasecmp (Doc->CurURL.schema, "https"))
                    continue;
                  if (strncasecmp (DpsSQLValue (&Res, i, 2), Doc->CurURL.path, dps_strlen (DpsSQLValue (&Res, i, 2))))
                    continue;
                  if (cookie.data_size)
                    DpsDSTRAppend (&cookie, "; ", 2);
