...
void __DPSCALL
DpsConvInit (DPS_CONV *cnv, DPS_CHARSET *from, DPS_CHARSET *to, const char *CharsToEscape, int fl)
{
  cnv->from = from;
  cnv->to = to;
  cnv->flags = fl;
...
static int
dps_ENTITYprint (char *dest, char sym, dpsunicode_t n)
{
...
static int
dps_JSONprint (char *dest, dpsunicode_t n)
{
...
#ifdef DEBUG_CONV
int __DPSCALL
_DpsConv (DPS_CONV *c, char *d, size_t dlen, const char *s, size_t slen, const char *filename, int line)
{
#else
int __DPSCALL
DpsConv (DPS_CONV *c, char *d, size_t dlen, const char *s, size_t slen)
{
#endif
  size_t i, codes;
  int res;
  ...
  while (s < s_e && d < d_e)
    {
      ...
      else if ((res == DPS_CHARSET_ILSEQ) || (res == DPS_CHARSET_ILSEQ2) || (res == DPS_CHARSET_ILSEQ3) || (res == DPS_CHARSET_ILSEQ4) || (res == DPS_CHARSET_ILSEQ5) || (res == DPS_CHARSET_ILSEQ6))
        {

          switch (res)
            {
            case DPS_CHARSET_ILSEQ6:
              s++;
            case DPS_CHARSET_ILSEQ5:
              s++;
            case DPS_CHARSET_ILSEQ4:
              s++;
            case DPS_CHARSET_ILSEQ3:
              s++;
            case DPS_CHARSET_ILSEQ2:
              s++;
...
int __DPSCALL
DpsNConv (DPS_CONV *c, size_t n, char *d, size_t dlen, const char *s, size_t slen)
{
  size_t i, codes, nw = 0;
  int res;
  dpsunicode_t wc[32]; /* Is 32 enough? */
...
extern size_t
DpsUniConvLength (DPS_CONV *c, const char *s)
{
...
DpsCsGroup (const DPS_CHARSET *cs)
{
  switch (cs->family)
    {
    case DPS_CHARSET_ARABIC:
      return "Arabic";
    case DPS_CHARSET_ARMENIAN:
      return "Armenian";
    case DPS_CHARSET_BALTIC:
      return "Baltic";
    case DPS_CHARSET_CELTIC:
      return "Celtic";
    case DPS_CHARSET_CENTRAL:
      return "Central European";
    case DPS_CHARSET_CHINESE_SIMPLIFIED:
      return "Chinese Simplified";
    case DPS_CHARSET_CHINESE_TRADITIONAL:
      return "Chinese Traditional";
    case DPS_CHARSET_CYRILLIC:
      return "Cyrillic";
    case DPS_CHARSET_GREEK:
      return "Greek";
    case DPS_CHARSET_HEBREW:
      return "Hebrew";
    case DPS_CHARSET_ICELANDIC:
      return "Icelandic";
    case DPS_CHARSET_JAPANESE:
      return "Japanese";
    case DPS_CHARSET_KOREAN:
      return "Korean";
    case DPS_CHARSET_NORDIC:
      return "Nordic";
    case DPS_CHARSET_SOUTHERN:
      return "South Eur";
    case DPS_CHARSET_THAI:
      return "Thai";
    case DPS_CHARSET_TURKISH:
      return "Turkish";
    case DPS_CHARSET_UNICODE:
      return "Unicode";
    case DPS_CHARSET_VIETNAMESE:
      return "Vietnamese";
    case DPS_CHARSET_WESTERN:
      return "Western";
    case DPS_CHARSET_GEORGIAN:
      return "Georgian";
    case DPS_CHARSET_INDIAN:
      return "Indian";
    case DPS_CHARSET_LAO:
      return "Lao";
    case DPS_CHARSET_IRANIAN:
      return "Iranian";
    case DPS_CHARSET_TAJIK:
      return "Tajik";
    default:
      return "Unknown";
    }
