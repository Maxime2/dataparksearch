...
static void
DpsPrintLangMap (DPS_LANGMAP *map)
{
  double ratio;
  size_t i, minv;
...
main (int argc, char **argv)
{
  ...
  for (t = 0; t <= DPS_LM_HASHMASK; t++)
    mchar.memb3[t].index = mchar.memb6[t].index = t;
...
#ifdef DEBUG_GUESSER
  {
    ...
    for (i = 0; i < DPS_LM_HASHMASK; i++)
      {
        if (mchar.memb3[i].count == 0)
          count0 += 1.0;
        if (mchar.memb6[i].count == 0)
          count0 += 1.0;
      }
  ...
  if (print)
  ...
  else
    {
      ...
      if (env.nmaps > 1)
        DpsSort (mapstat, env.nmaps, sizeof (DPS_MAPSTAT), &DpsLMstatcmp);
