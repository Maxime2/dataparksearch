...
DpsGetSemLimit (void)
{
...
#elif defined(__OpenBSD__)
  {
    ...
    if (sysctl (mib, 3, &maxsem, &len, NULL, 0) != -1)
      {
        if (maxsem != -1)
          DpsNsems = (size_t) maxsem;
        if (DpsNsems == 0)
          DpsNsems = 0x300 + DPS_LOCK_MAX;
      }
  }
...
static inline char
CAS (dps_mutex_t *target, dps_mutex_t exchange, dps_mutex_t compare)
{
  char ret;
...
void
DpsCAS_lock (DPS_AGENT *A, dps_mutex_t *mut)
{
  while (!CAS (mut, A, NULL))
    {
      DPS_MSLEEP (50 + random () % 20);
    }
}

void
DpsCAS_unlock (DPS_AGENT *A, dps_mutex_t *mut)
{
...
void
DPS_MUTEX_LOCK (DPS_AGENT *A, dps_mutex_t *x)
{
...
void
DPS_MUTEX_UNLOCK (DPS_AGENT *A, dps_mutex_t *x)
{
...
__C_LINK void __DPSCALL
DpsLockProc (DPS_AGENT *A, int command, size_t type, const char *fn, int ln)
{
...
/*			assert(A->Locked[type] == 1);*/
    ...
    case DPS_UNLOCK:
      ...
      u = (A->Locked[type] == 0);
      if (u)
        DPS_MUTEX_UNLOCK (A, &MuMu[type].mutex);
      break;
    }
#ifdef DEBUG_LOCK
  if (type != DPS_LOCK_THREAD)
...
void
DpsAcceptMutexCleanup (void)
{
  if (accept_mutex != (dps_mutex_t *) -1 && munmap ((void *) accept_mutex, sizeof (*accept_mutex)))
    {
      perror ("munmap");
    }
...
static void
dps_remove_sync_sigs (sigset_t *sig_mask)
{
#ifdef SIGABRT
...
void
DpsAcceptMutexInit (const char *var_dir, const char *app)
{
  int fd;
...
void
DpsAcceptMutexLock (DPS_AGENT *Agent)
{
...
void
DpsAcceptMutexUnlock (DPS_AGENT *Agent)
{
...
static void
id_function (CRYPTO_THREADID *id)
{
...
int
dps_libcrypto_thread_setup (void)
{
#if defined WITH_OPENSSL
  int i;
...
int
dps_libcrypto_thread_cleanup (void)
{
#if defined WITH_OPENSSL
  int i;
