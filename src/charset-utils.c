...
{
  if (c < 0x80)
    return (int) DpsUniToLower ((dpsunicode_t) c);
  return tolower (c);
}

char *
dps_strtolower (char *str)
{
  register char *s = str;
  if (s != NULL)
    {
...
void
dps_mstr (char *s, const char *src, size_t l1, size_t l2)
{
  l1 = l1 < l2 ? l1 : l2;
...
DPS_DSTR *
DpsDSTRInit (DPS_DSTR *dstr, size_t page_size)
{
  if (page_size == 0)
    return NULL;
...
size_t
DpsDSTRAppend (DPS_DSTR *dstr, const void *data, size_t append_size)
{
  size_t bytes_left = (dstr->allocated_size - dstr->data_size);
  size_t asize;
  char *dstr_data;

  if (data == NULL || append_size == 0)
    return 0;
  ...
  if (bytes_left <= append_size + 2 * sizeof (dpsunicode_t))
    {
      ...
      if (dstr->data == NULL)
        {
          dstr->allocated_size = dstr->data_size = 0;
          return 0;
        }
...
size_t
DpsDSTRAppendStr (DPS_DSTR *dstr, const char *data)
{
  return DpsDSTRAppend (dstr, data, dps_strlen (data));
}

size_t
DpsDSTRAppendStrWithSpace (DPS_DSTR *dstr, const char *data)
{
  char space[] = { 0x20, 0 };
  size_t rc;
...
size_t
DpsDSTRAppendUniStr (DPS_DSTR *dstr, const dpsunicode_t *data)
{
  return DpsDSTRAppend (dstr, data, sizeof (dpsunicode_t) * DpsUniLen (data));
}

size_t
DpsDSTRAppendUni (DPS_DSTR *dstr, const dpsunicode_t data)
{
...
size_t
DpsDSTRAppendUniWithSpace (DPS_DSTR *dstr, const dpsunicode_t *data)
{
