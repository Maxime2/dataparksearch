...
static void
DpsUniDesegment (dpsunicode_t *s)
{
...
#ifdef HAVE_ASPELL
static void
DpsSpellSuggest (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, DPS_TEXTITEM *Item, dpsunicode_t *uword, size_t uwlen, int crossec, AspellSpeller *speller, DPS_DSTR *suggest, int *spelling)
{
  DPS_WORD Word;
  register int ii;
  ...
  if (ii == 0)
    {
      ...
      if (ii > 0 && suggest != NULL)
        DpsDSTRAppendUniStr (suggest, PERIOD);
...
static void
DpsProcessFantoms (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, DPS_TEXTITEM *Item, size_t min_word_len, int crossec, int have_bukva_forte, dpsunicode_t *uword, int make_prefixes, int make_suffixes, int strict
#ifdef HAVE_ASPELL
                   ,
                   int have_speller,
...
)
{
  ...
  if (strict &&
      have_speller && have_bukva_forte && Indexer->Flags.use_aspellext && (uwlen > 2) && (DpsUniStrChr (uword, (dpsunicode_t) '&') == NULL) /* aspell trap workaround */
  )
...
#ifdef HAVE_ASPELL
  if (spelling)
    DpsVarListReplaceStr (&Doc->Sections, "spelling", "1");
#endif
  TRACE_OUT (Indexer);
}

int
DpsPrepareItem (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, DPS_TEXTITEM *Item, dpsunicode_t *ustr, dpsunicode_t *UStr, const char *content_lang, size_t *indexed_size, size_t *indexed_limit, size_t max_word_len, size_t min_word_len, int crossec
#ifdef HAVE_ASPELL
                ,
                int have_speller,
                AspellSpeller *speller,
...
)
{
  ...
  if (Sec)
    {
      if (Sec->single && (Sec->val != NULL) && (Sec->curlen != 0))
        { /* Already have single valued section, skip the rest from processing */
          TRACE_OUT (Indexer);
          return DPS_OK;
        }
    }
  ...
  if (dps_need2segment (nfc))
    {
      if ((Indexer->Flags.Resegment & DPS_RESEGMENT_CHINESE) && !strncasecmp (content_lang, "zh", 2))
        DpsUniDesegment (nfc);
      else if ((Indexer->Flags.Resegment & DPS_RESEGMENT_JAPANESE) && !strncasecmp (content_lang, "ja", 2))
        DpsUniDesegment (nfc);
      else if ((Indexer->Flags.Resegment & DPS_RESEGMENT_KOREAN) && !strncasecmp (content_lang, "ko", 2))
        DpsUniDesegment (nfc);
      else if ((Indexer->Flags.Resegment & DPS_RESEGMENT_THAI) && !strncasecmp (content_lang, "th", 2))
        DpsUniDesegment (nfc);
...
int
dps_itemptr_cmp (DPS_TEXTITEM **p1, DPS_TEXTITEM **p2)
{
  if ((*p1)->len > (*p2)->len)
    return -1;
  if ((*p1)->len < (*p2)->len)
...
int
DpsPrepareWords (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
  size_t i;
  const char *doccset;
  DPS_CHARSET *doccs;
  ...
  if ((lcsword = (char *) DpsMalloc (12 * uwordlen + 1)) == NULL)
    {
      DPS_FREE (uword);
      DpsDSTRFree (&exrpt);
      TRACE_OUT (Indexer);
      ...
      if (items != NULL)
        {
          for (i = 0; i < tlist->nitems; i++)
            items[i] = &tlist->Items[i];
          DpsSort (items, tlist->nitems, sizeof (DPS_TEXTITEM *), (qsort_cmp) dps_itemptr_cmp);
          for (i = 0; (i < tlist->nitems) && (i < (size_t) Indexer->Flags.LongestTextItems); i++)
            items[i]->marked = 1;
        }
      DPS_FREE (items);
    }
...
int
DpsParseURLText (DPS_AGENT *A, DPS_DOCUMENT *Doc)
{
  DPS_TEXTITEM Item;
  DPS_URL dcURL;
  DPS_CONV lc_dc;
...
*/
int
DpsParseText (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
  DPS_TEXTITEM Item;
...
void
DpsHTMLTOKInit (DPS_HTMLTOK *tag)
{
...
void
DpsHTMLTOKFree (DPS_HTMLTOK *tag)
{
  /*  register size_t i;
...
const char *
DpsHTMLToken (const char *s, const char **lt, DPS_HTMLTOK *t)
{
  ...
  switch (t->type)
      ...
      for (*lt = t->b = t->s + 1; *t->b;)
        {
          ...
          if (nt == 0)
            {
              if (!strncasecmp (t->b, "script", 6))
                t->script = 1;
              else if (!strncasecmp (t->b, "/script", 7))
                {
                  t->script = 0;
                  if (t->comment_inside)
                    {
                      t->comment = t->comment_inside = 0;
                    }
                }
              else if (!strncasecmp (t->b, "style", 5))
                t->style = 1;
              else if (!strncasecmp (t->b, "/style", 6))
                {
                  t->style = 0;
                  if (t->comment_inside)
                    {
              ...
              else if (!strncasecmp (t->b, "select", 4))
                t->select = 1;
              else if (!strncasecmp (t->b, "/select", 5))
                t->select = 0;
              else if (!strncasecmp (t->b, "body", 4))
                t->body = 1;
              else if (!strncasecmp (t->b, "/body", 5))
                t->body = 0;
              else if (!strncasecmp (t->b, "title", 5))
                t->title = 1;
              else if (!strncasecmp (t->b, "/title", 6))
                t->title = 0;
              else if (!strncasecmp (t->b, "noindex", 7))
                t->noindex = 1;
              else if (!strncasecmp (t->b, "/noindex", 8))
                t->noindex = 0;
              else if (!strncasecmp (t->b, "frameset", 8))
                t->frameset++;
              else if (!strncasecmp (t->b, "/frameset", 9))
                {
                  if (t->frameset)
                    t->frameset--;
                }
              else if (!strncasecmp (t->b, "br", 2))
                {
                  t->br++;
                }
            }
    ...
    case DPS_HTML_COM: /* comment */
      ...
      if (!strncasecmp (t->s, "<!--DpsComment-->", 17))
        t->noindex = 1;
      else if (!strncasecmp (t->s, "<!--/DpsComment-->", 18))
        t->noindex = 0;
      else if (!strncasecmp (t->s, "<!--UdmComment-->", 17))
        t->noindex = 1;
      else if (!strncasecmp (t->s, "<!--/UdmComment-->", 18))
        t->noindex = 0;
      else if (!strncasecmp (t->s, "<!--noindex-->", 14))
        t->noindex = 1;
      else if (!strncasecmp (t->s, "<!--/noindex-->", 15))
        t->noindex = 0;
      else if (!strncasecmp (t->s, "<!--Comment-->", 14))
        t->noindex = 1;
      else if (!strncasecmp (t->s, "<!--/Comment-->", 15))
        t->noindex = 0;
      else if (!strncasecmp (t->s, "<!--htdig_noindex-->", 20))
        t->noindex = 1;
      else if (!strncasecmp (t->s, "<!--/htdig_noindex-->", 21))
        t->noindex = 0;
      else if (!strncasecmp (t->s, "<!-- google_ad_section_start -->", 32))
        t->noindex = 0;
      else if (!strncasecmp (t->s, "<!-- google_ad_section_start(weight=ignore) -->", 47))
        t->noindex = 1;
      else if (!strncasecmp (t->s, "<!-- google_ad_section_end -->", 30))
        t->noindex = !(t->noindex);
...
static int
DpsHasEndTag (const char *name)
{
  if (!strcasecmp (name, "img"))
    return 0;
  if (!strcasecmp (name, "br"))
    return 0;
  if (!strcasecmp (name, "hr"))
    return 0;
  ...
  if (!strcasecmp (name, "area"))
    return 0;
  if (!strcasecmp (name, "base"))
    return 0;
  if (!strcasecmp (name, "basefont"))
    return 0;
  if (!strcasecmp (name, "col"))
    return 0;
  if (!strcasecmp (name, "frame"))
    return 0;
  if (!strcasecmp (name, "input"))
    return 0;
  if (!strcasecmp (name, "isindex"))
    return 0;
...
static void
putItem (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, DPS_TEXTITEM *Item)
{
  DPS_TEXTITEM *Last;
  if (Doc->TextList.nitems == 0)
  ...
  else
    {
      ...
      if (Item->section == Last->section && ((Item->href == NULL && Last->href == NULL) || strcmp (DPS_NULL2EMPTY (Item->href), DPS_NULL2EMPTY (Last->href)) == 0))
        {
          if ((Last->str = (char *) DpsRealloc (Last->str, Last->len + Item->len + 1)) == NULL)
            return;
...
int
DpsHTMLParseTag (DPS_AGENT *Indexer, DPS_HTMLTOK *tag, DPS_DOCUMENT *Doc, DPS_VAR *CrosSec)
{
  DPS_TEXTITEM Item;
  DPS_VAR *Sec;
      ...
      if ((strcasecmp (name, "hr")) && (strcasecmp (name, "br")) && (strcasecmp (name, "font")))
        {
          ...
          if (tag->level < sizeof (tag->visible) - 1)
            visible = tag->visible[tag->level + 1] = tag->visible[tag->level];
  ...
  for (i = 0; i < tag->ntoks; i++)
    {
      if (ISTAG (i, "name") || ISTAG (i, "http-equiv") || ISTAG (i, "property"))
        {
          DPS_FREE (metaname);
          if (tag->toks[i].vlen)
            metaname = DpsStrndup (DPS_NULL2EMPTY (tag->toks[i].val), tag->toks[i].vlen);
        }
      else if (ISTAG (i, "content"))
        {
          DPS_FREE (metacont);
          if (tag->toks[i].vlen)
            metacont = DpsStrndup (DPS_NULL2EMPTY (tag->toks[i].val), tag->toks[i].vlen);
          metacont_len = tag->toks[i].vlen;
        }
      else if (ISTAG (i, "href"))
        {
          /* A, LINK, AREA*/
      ...
      else if (ISTAG (i, "data-expanded-url"))
        {
          /* Twitter's A */
          char *y = DpsStrndup (DPS_NULL2EMPTY (tag->toks[i].val), tag->toks[i].vlen);
      ...
      else if (ISTAG (i, "data-ultimate-url"))
        {
          /* Twitter's A */
          char *y = DpsStrndup (DPS_NULL2EMPTY (tag->toks[i].val), tag->toks[i].vlen);
      ...
      else if (Indexer->Flags.rel_nofollow && ISTAG (i, "rel"))
        {
          char *y = DpsStrndup (DPS_NULL2EMPTY (tag->toks[i].val), tag->toks[i].vlen);
      ...
      else if (ISTAG (i, "style") && DpsHasEndTag (name))
        {
          char *y = DpsStrndup (DPS_NULL2EMPTY (tag->toks[i].val), tag->toks[i].vlen);
  ...
  else if (!strcmp (name, "html") && opening && (lang != NULL))
    {
  ...
  else if ((!strcmp (name, "meta")) && (charset))
    {
  ...
  else if ((!strcmp (name, "meta")) && (metaname) && (metacont))
    {
      ...
      else if (!strcasecmp (metaname, "Content-Language") || !strcasecmp (metaname, "DC.Language"))
        {
          char *l;
      ...
      else if (!strcasecmp (metaname, "robots") && (Doc->Spider.use_robots) && (metacont))
        {
          char *lt;
          char *rtok;
      ...
      else if (!strcasecmp (metaname, "DP.PopRank"))
        {
          char pstr[32];
      ...
      else if (!strcasecmp (metaname, "Last-Modified"))
        {
          DpsVarListReplaceStr (&Doc->Sections, metaname, metacont);
        }
      else if (!strcasecmp (metaname, "Date"))
        {
          DpsVarListReplaceStr (&Doc->Sections, metaname, metacont);
        }
      else if (!strcasecmp (metaname, "geo.position"))
        {
          double lat, lon;
      ...
      else if (!strcasecmp (metaname, "ICBM"))
        {
          double lat, lon;
          char *l = strchr (metacont, (int) ',');
          if (l != NULL)
      ...
      else if (strcasecmp (metaname, "title") && strcasecmp (metaname, "category") && strcasecmp (metaname, "tag") && strcasecmp (metaname, "url"))
        {
  ...
  if ((href || data_expanded_url || data_ultimate_url) && visible && (Doc->Spider.follow != DPS_FOLLOW_NO) && (tag->follow != DPS_FOLLOW_NO) && !(Indexer->Flags.SkipHrefIn & DpsHrefFrom (name)))
    {
      ...
      if (url_pointer == NULL)
        url_pointer = data_expanded_url;
      if (url_pointer == NULL)
        url_pointer = href;

      if ((Doc->subdoc < Indexer->Flags.SubDocLevel) && (Doc->sd_cnt < Indexer->Flags.SubDocCnt) && (!strcasecmp (name, "IFRAME") || !strcasecmp (name, "EMBED") || !strcasecmp (name, "FRAME") || !strcasecmp (name, "META")))
        {
...
int
DpsHTMLParseBuf (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc, const char *section_name, const char *content)
{
  DPS_HTMLTOK tag;
  DPS_TEXTITEM Item;
  const char *htok;
      ...
      */
      switch (tag.type)
        ...
        case DPS_HTML_TXT:
          ...
          if (BSec && (tag.comment + tag.noindex == 0) && !tag.title && (tag.body || tag.frameset) && !tag.script && !tag.style && tag.index && !tag.select && tag.visible[tag.level])
            {
              int z;
...
int
DpsHTMLParse (DPS_AGENT *Indexer, DPS_DOCUMENT *Doc)
{
