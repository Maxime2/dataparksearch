...
static int
DpsRobotCmp (DPS_ROBOT *r1, DPS_ROBOT *r2)
{
  return strcasecmp (r1->hostinfo, r2->hostinfo);
}

DPS_ROBOT *
DpsRobotFind (DPS_ROBOTS *Robots, const char *hostinfo)
{
  DPS_ROBOT *r, key;
  ...
  if (Robots->nrobots == 0)
    return NULL;
  if (Robots->nrobots == 1)
    return (strcasecmp (Robots->Robot->hostinfo, hostinfo) == 0) ? Robots->Robot : NULL;
...
static DPS_ROBOT *
DeleteRobotRules (DPS_AGENT *A, DPS_ROBOTS *Robots, const char *hostinfo)
{
  DPS_ROBOT *robot;
  size_t i;
#ifdef WITH_PARANOIA
...
static DPS_ROBOT *
DpsRobotAddEmpty (DPS_AGENT *A, DPS_ROBOTS *Robots, const char *hostinfo, DPS_ROBOT_CRAWL *last_crawled)
{
  DPS_ROBOT *r;
#ifdef WITH_PARANOIA
  void *paran = DpsViolationEnter (paran);
...
static int
AddRobotRule (DPS_AGENT *A, DPS_ROBOT *robot, int cmd, const char *path, int insert_flag)
{
#ifdef HAVE_SQL
  DPS_DB *db;
  dpshash32_t url_id;
...
int
DpsRobotListFree (DPS_AGENT *A, DPS_ROBOTS *Robots)
{
  size_t i, j;
#ifdef WITH_PARANOIA
  void *paran = DpsViolationEnter (paran);
...
static DPS_ROBOT *
DpsRobotClone (DPS_AGENT *Indexer, DPS_SERVER *Server, DPS_DOCUMENT *Doc, DPS_URL *URL, char *rurl, size_t rurlen)
{
  DPS_ROBOTS *Robots;
  ...
  if (robot == NULL)
    {
      ...
      if (Server != NULL)
        ...
        rServer = DpsServerFind (Indexer, (urlid_t) DpsVarListFindInt (&Doc->Sections, "Server_id", 0), rurl, URL->charset_id, NULL);
      ...
      if (Doc != NULL)
        bzero (&rDoc->connp, sizeof (rDoc->connp));
      DpsDocFree (rDoc);
    }
  ...
  if (robot != NULL)
    {
      ...
      if (rI == NULL)
        rI = DpsRobotAddEmpty (Indexer, &Indexer->Robots, DPS_NULL2EMPTY (URL->hostinfo), robot->last_crawled);
      if (rI != NULL)
        {
          size_t j;
...
DPS_ROBOT_RULE *
DpsRobotRuleFind (DPS_AGENT *Indexer, DPS_SERVER *Server, DPS_DOCUMENT *Doc, DPS_URL *pURL, int make_pause, int aliased)
{
  ...
  if (strncasecmp (DPS_NULL2EMPTY (URL->schema), "http", 4))
    { /* robots.txt exists only for http and https */
#ifdef WITH_PARANOIA
      DpsViolationExit (Indexer->handle, paran);
#endif
      return NULL;
...
static int
Text (DPS_XML_PARSER *parser, const char *s, size_t len)
{
...
static int
DpsSitemapEndElement (DPS_XML_PARSER *parser, const char *name, size_t l)
{
  XML_PARSER_DATA *D = parser->user_data;
  size_t i = l;
  char *p;
...
static int
DpsSitemapParse (DPS_AGENT *Indexer, int hops, const char *s)
{
  char reason[1024] = "";
...
static char *
dps_robots_normalise (const char *s)
{ /* robots.txt path normalisation, blame to Google */
...
int
DpsRobotParse (DPS_AGENT *Indexer, DPS_SERVER *Srv, const char *content, const char *hostinfo, int hops)
{
  ...
  const char *UA = (Srv != NULL) ? DpsVarListFindStr (&Srv->Vars, "Request.User-Agent", DPS_USER_AGENT) : DpsVarListFindStr (&Indexer->Vars, "Request.User-Agent", DPS_USER_AGENT);

  /* Wipe out any existing (default) rules for this host */
  DPS_GETLOCK (Indexer, DPS_LOCK_ROBOTS);
  robot = DeleteRobotRules (Indexer, Robots, DPS_NULL2EMPTY (hostinfo));
  if (robot == NULL)
    robot = DpsRobotAddEmpty (Indexer, Robots, DPS_NULL2EMPTY (hostinfo), NULL);
  if (robot == NULL)
    {
      DPS_RELEASELOCK (Indexer, DPS_LOCK_ROBOTS);
      return (DPS_ERROR);
  ...
  */
  s = (char *) content;
  while (*s && (*s == NL_CHAR || *s == CR_CHAR))
    s++;
  lt = s;
  while (*lt && (*lt != NL_CHAR) && (*lt != CR_CHAR))
    lt++;
  if (*lt == CR_CHAR)
    *lt++ = '\0';
  if (*lt)
      ...
      */
      ...
      if (*s == '#')
      ...
      else
        {
          ...
          else if ((!(strncasecmp (s, "Host", 4))) && (rule))
            {
              if ((e = strchr (s + 5, '#')))
                *e = 0;
              e = s + 5;
          ...
          else if ((!(strncasecmp (s, "Sitemap", 7))) && (rule) && use_sitemaps)
            {
              if ((e = strchr (s + 8, '#')))
                *e = 0;
              e = s + 8;
          ...
          else if ((!(strncasecmp (s, "Crawl-delay", 11))) && (rule))
            {
              e = s + 12;
              DPS_SKIP (e, " \t");
              s = e;
