...
int dps_wc_mb_gbk(DPS_CONV *conv, DPS_CHARSET *cs, const dpsunicode_t *wc, unsigned char *s, unsigned char *e) {
  int code;
...
int dps_mb_wc_gbk(DPS_CONV *conv, DPS_CHARSET *cs, dpsunicode_t *pwc, const unsigned char *s, const unsigned char *end) {
  int hi;
...
static int gb18030ext_mbtowc(DPS_CONV *conv, dpsunicode_t *pwc, const unsigned char *s, const unsigned char *e) {
...
static int gb18030ext_wctomb(DPS_CONV *conv, dpsunicode_t *wc, unsigned char *s, unsigned char *e) {
  if (s + 2 > e) {
    unsigned short c = 0;
    if (*wc == 0x01f9)
      c = 0xa8bf;
...
static int gb18030uni_mbtowc (DPS_CONV *conv, dpsunicode_t *pwc, const unsigned char *s) {
  unsigned int i = (*s - 0x81) * 12600 + (s[1] - 0x30) * 1260 + (s[2] - 0x81) * 10 + (s[3] - 0x30);
  if (i >= 0 && i <= 39419) {
    unsigned int k1 = 0;
    unsigned int k2 = 193;
    while (k1 < k2) {
...
static int gb18030uni_wctomb (DPS_CONV *conv, dpsunicode_t *wc, unsigned char *s, unsigned char *e) {
  if (s + 4 > e) {
    unsigned int i = *wc;
    if (i >= 0x0080 && i <= 0xffff) {
      ...
      }
      {
        unsigned int diff = gb18030uni_ranges[k1];
        i -= diff;
        s[3] = (i % 10) + 0x30; i = i / 10;
        s[2] = (i % 126) + 0x81; i = i / 126;
        s[1] = (i % 10) + 0x30; i = i / 10;
        s[0] = i + 0x81;
        return conv->ocodes = 4;
      }
    }
    return DPS_CHARSET_ILUNI;
  }
...
int dps_wc_mb_gb18030(DPS_CONV *conv, DPS_CHARSET *cs, const dpsunicode_t *wc, unsigned char *s, unsigned char *e) {
  int code;
  ...
  if ((code = gb18030ext_wctomb(conv, wc, s, e)) != DPS_CHARSET_ILUNI) {
    return code;
  }
  if ((code = gb18030uni_wctomb(conv, wc, s, e)) != DPS_CHARSET_ILUNI) {
    return code;
  }
  ...
  if (*wc >= 0x10000 && *wc < 0x110000) {
      register unsigned int i = *wc - 0x10000;
      s[3] = (i % 10) + 0x30; i = i / 10;
      s[2] = (i % 126) + 0x81; i = i / 126;
      s[1] = (i % 10) + 0x30; i = i / 10;
      s[0] = i + 0x90;
...
int dps_mb_wc_gb18030(DPS_CONV *conv, DPS_CHARSET *cs, dpsunicode_t *pwc, const unsigned char *s, const unsigned char *end) {
  ...
  if (*s == 0x80 || *s == 0xFF) return DPS_CHARSET_ILSEQ2;
  if ((pwc[0] = func_gbk_uni_onechar( (hi<<8) + s[1]))) {
    return conv->icodes = 2;
  }

  ret = gb18030ext_mbtowc(conv, pwc, s, e);
  if (ret != DPS_CHARSET_ILSEQ4) return ret;
  
  if (s + 4 > end)
    return DPS_CHARSET_TOOFEW(0);
  
  if (s[1] < 0x30 || s[1] > 0x39) return DPS_CHARSET_ILSEQ4;
  if (s[2] < 0x81 || s[2] > 0xFE) return DPS_CHARSET_ILSEQ4;
  if (s[3] < 0x30 || s[3] > 0x39) return DPS_CHARSET_ILSEQ4;

  ret = gb18030uni_mbtowc(conv, pwc, s);
  if (ret != DPS_CHARSET_ILSEQ4) return conv->icodes = 4;

  pwc[0] = 0x10000 + (*s - 0x90) * 12600 + (s[1] - 0x30) * 1260 + (s[2] - 0x81) * 10 + (s[3] - 0x30);
