  ...
  for (z = 0; z < 90; z++)
    {
      ...
      if (rfd <= 0)
        break;
      if (read (socket, junk, sizeof (junk)) <= 0)
        break;
    }
  (void) close (socket);
}
...
int
socket_select (DPS_CONN *connp, size_t timeout, int mode)
{
  fd_set fds;
  struct timeval tv;
  int rfd;
...
int
socket_read (DPS_CONN *connp, size_t maxsize)
{
  int num_read;
  ...
  do
    {
      ...
      if (num_read < 0)
        ...
        {
          if ((size_t) (time (NULL) - t) > connp->timeout)
            break;
        }
      else
        t = time (NULL);
...
int
socket_read_line (DPS_CONN *connp)
{
...
int
socket_write (DPS_CONN *connp, const char *buf)
{
...
int
socket_getname (DPS_CONN *connp, struct sockaddr_in *sa_in)
{
  socklen_t len;
...
void
socket_buf_clear (DPS_CONN *connp)
{
  char buf[1024];
  int len;
  do
...
ssize_t
DpsRecvall (int s, void *buf, size_t len, size_t time_to_wait)
{
  ...
  while ((size_t) received < len)
    {
      ...
      if (r == 0)
        {
          if (time_to_wait > 0 && ((size_t) (time (NULL) - start) > time_to_wait))
            break;
          DPS_MSLEEP (1);
        }
    }
...
ssize_t
DpsRecvstr (int s, void *buf, size_t len, size_t time_to_wait)
{
  ...
  while (notdone && (size_t) received < len)
    {
      ...
      if ((r = read (s, b + received, dps_min (len - received, DPS_BLKLEN))) > 0)
        {
          for (i = 0; i < r; i++)
            if (b[received + i] == '\0' || b[received + i] == '\n')
              notdone = 0;
          received += r;
        }
      if (((r < 0) && (errno != EINTR)) /* || have_sigint || have_sigterm */
...
ssize_t
DpsSend (int s, const void *msg, size_t len, int flags)
{
  ssize_t o = 0;
  size_t olen, slen;
  const char *p = msg;
...
void
DpsSockOpt (DPS_AGENT *A, int dps_socket)
{
  const int lowat = 1;
...
void
DpsDocSockOpt (DPS_AGENT *A, DPS_DOCUMENT *Doc, int dps_socket)
{
#if !defined(sgi) && !defined(__sgi) && !defined(__irix__) && !defined(sun) && !defined(__sun)
  struct timeval so_tval;
...
int
DpsSockPrintf (int *socket, const char *fmt, ...)
{
  char buf[4096];
  va_list ap;
  size_t len;
  ...
  va_end (ap);
  len = dps_strlen (buf);
  if ((ssize_t) len != DpsSend (*socket, buf, len, 0))
    return DPS_ERROR;
  return DPS_OK;
}
