...
static int
DpsBaseWriteItem (DPS_BASE_PARAM *P, dps_uint8 ItemPos)
{
  ssize_t wr;
...
DpsBaseOpen (DPS_BASE_PARAM *P, int mode)
{
  unsigned int hash;
  ...
  if (P->NFiles == 0)
    P->NFiles = DpsVarListFindUnsigned (&P->A->Vars, "BaseFiles", 0x100);
  ...
  for (z = 0; z < 3; z++)
    {
      ...
      if (P->Item.rec_id == P->rec_id || P->Item.rec_id == 0)
        P->mishash = 0;
      else
        P->mishash = 1;
...
__C_LINK int __DPSCALL
DpsBaseSeek (DPS_BASE_PARAM *P, int mode)
{
  unsigned int hash;
...
__C_LINK int __DPSCALL
DpsBaseDelete (DPS_BASE_PARAM *P)
{
  int res = DPS_OK;
...
__C_LINK int __DPSCALL
DpsBaseWrite (DPS_BASE_PARAM *P, void *buffer, size_t len)
{
  ...
  if ((P->zlib_method == Z_DEFLATED) && (deflateInit2 (&zstream, P->zlib_level, Z_DEFLATED, P->zlib_windowBits, P->zlib_memLevel, P->zlib_strategy) == Z_OK))
    {
...
__C_LINK int __DPSCALL
DpsBaseRead (DPS_BASE_PARAM *P, void *buf, size_t len)
{
  int res = DPS_OK;
#ifdef HAVE_ZLIB
  z_stream zstream;
...
DpsBaseARead (DPS_BASE_PARAM *P, size_t *len)
{
  int res = DPS_OK;
  char *buf = NULL;
#ifdef HAVE_ZLIB
...
__C_LINK int __DPSCALL
DpsBaseCheckup (DPS_BASE_PARAM *P, int (*checkrec) (DPS_AGENT *A, const urlid_t rec_id))
{
  int found;
  urlid_t i;
  size_t z;
  ...
  for (i = 0; i < (urlid_t) P->NFiles; i++)
    {
      ...
      if (have_sigterm || have_sigint || have_sigalrm)
        {
          DpsLog (P->A, DPS_LOG_EXTRA, "%s signal received. Exiting chackup", (have_sigterm) ? "SIGTERM" : (have_sigint) ? "SIGINT"
                                                                                                                         : "SIGALRM");
          DpsBaseClose (P);
          DPS_FREE (todel);
          return DPS_OK;
...
extern __C_LINK int __DPSCALL
DpsBaseOptimize (DPS_BASE_PARAM *P, int sbase)
{
  struct stat sb;
      ...
      if ((dr >= (double) OptimizeRatio) || (ActualSize == 0 && SSize != 0))
        {
          ...
          if (i < nitems)
            nitems = i;
          if (nitems > 1)
            DpsSort ((void *) si, (size_t) nitems, sizeof (DPS_SORTBASEITEM), cmpsi);
          ...
          /*if (gain != 0 || OptimizeRatio == 0 || error_cnt > 0)*/ {
            ...
            for (i = 0; i < nitems; i++)
              {
                if (si[i].Item.rec_id == 0 || si[i].Item.size == 0)
                  continue;
...
extern __C_LINK int __DPSCALL
DpsBaseRelocate (DPS_AGENT *Agent, int base_type)
{
  DPS_BASE_PARAM O, N;
  ...
  for (base = 0; base < O.NFiles; base++)
    {
      ...
      if (have_sigterm || have_sigint || have_sigalrm)
        {
          DpsLog (Agent, DPS_LOG_EXTRA, "%s signal received. Exiting chackup", (have_sigterm) ? "SIGTERM" : (have_sigint) ? "SIGINT"
                                                                                                                          : "SIGALRM");
          DpsBaseClose (Old);
          DpsBaseClose (New);
          DPS_FREE (todel);
