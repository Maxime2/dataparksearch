...
(define (xref-strings)
  (list (list (normalize "appendix")    (if %chapter-autolabel%
					    "&Appendix.abr; %n"
					    "&Appendix.abr; %t"))
	(list (normalize "article")     (string-append %gentext-start-quote%
						       "%t"
						       %gentext-end-quote%))
...
(define (gentext-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (gentext-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
...
(define (local-intra-label-sep)
  (list))

(define (intra-label-sep)
  (list
...
(define (gentext-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-intra-label-sep)))
	 (sep    (assoc name (intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-label-title-sep)
  (list))

(define (label-title-sep)
  (list
...
(define (gentext-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-label-title-sep)))
	 (sep    (assoc name (label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (label-number-format-list)
  (list
...
(define (label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-start-nested-quote% (dingbat "lsquo"))

(define %gentext-end-nested-quote% (dingbat "rsquo"))

(define %gentext-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe
...
(define %gentext-listcomma% "&listcomma;")

(define %gentext-lastlistcomma% "&lastlistcomma;")

(define %gentext-bibl-pages% "&Pgs;")

(define %gentext-endnotes% "&Notes;")

(define %gentext-table-endnotes% "&TableNotes;:")
